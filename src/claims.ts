...
export async function validateStageClaims(
    ...
    for (const reference of claim.evidenceRefs) {
      ...
      if (reference === openSpecCitation) {
        if (claim.kind !== "current-spec-decision" || claim.maturity !== "fact") {
          findings.push(finding("OPENSPEC_INDEX_CLAIM_TYPE_INVALID", base,
            "OpenSpec 索引只能登记为 current-spec-decision/fact，不能伪装成业务开放问题或目标设计。"))
        }
        if (!openSpecStatement || claim.statement.trim() !== openSpecStatement) {
          findings.push(finding("OPENSPEC_INDEX_SUBJECT_MISMATCH", `${base}.statement`,
            `OpenSpec 索引 claim 必须逐字使用签发 statement：“${openSpecStatement || "<缺少签发 statement>"}”。`))
        }
        if (!Array.isArray(claim.authorityRefs) || !claim.authorityRefs.includes(reference)) {
          findings.push(finding("OPENSPEC_INDEX_AUTHORITY_MISMATCH", `${base}.authorityRefs`,
  ...
  for (const part of narrativeParts) {
    ...
    if (openSpecStatement && part.text.includes(openSpecStatement)
      ...
      findings.push(finding(
        "UNMAPPED_OPENSPEC_INDEX_EVIDENCE",
        part.path,
        "正文使用了签发的 OpenSpec 索引 statement，但完整 claims 中没有对应 current-spec-decision/fact。",
        "保留逐字匹配的 typed claim，或同时删除正文中的索引 statement。",
      ))
    }
