...
export const viewGroups: string[][] = [
...
export const clean = (text: string) => text.replace(/<!--[\s\S]*?-->/gu, "").replace(/\x1b\[[0-9;]*[A-Za-z]/gu, "").replace(/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/gu, "").trim()
export interface VisualEdge { from: string; to: string; label: string; candidate: boolean }
export interface VisualCard { title: string; body: string; kind: "event" | "domain" | "model" | "evidence" | "warning" | "info"; edges?: VisualEdge[] }

export function explicitEdges(source: string): VisualEdge[] {
  const labels = new Map<string, string>()
  for (const match of source.matchAll(/([\w-]+)\s*\[\s*["']?([^\]\n]+?)["']?\s*\]/gu)) labels.set(match[1], match[2].replace(/<br\s*\/?\s*>/giu, " "))
  const node = "([\\w-]+)(?:\\s*\\[[^\\]\\n]*\\])?"
...
export function diagramText(source: string): string {
  const lines = explicitEdges(source).map(e => `${e.from} ${e.candidate ? "┄候选┄▷" : "──▶"} ${e.label ? `[${e.label}] ` : ""}${e.to}`)
  return lines.length ? `${lines.join("\n")}\n\n原始图（保留全部分组、节点和连线）：\n${source}` : `原始图（当前图语法以文本展示）：\n${source}`
}

export function visualCards(item: WorkflowItem, view: MilestoneView, index: number): VisualCard[] {
  if (view.projection) return view.projection.cards
  if (view.revisions?.length) return [{ title: "投影一致性异常", body: "请核对正式文档与当前投影；不会从冲突内容重新推测图。", kind: "warning" }]
  if (!view.checkpoint) return [{ title: "尚未开始", body: "本里程碑尚无已提交产物。请按工作流顺序完成前置阶段。", kind: "info" }]
  ...
  for (const heading of viewGroups[index]) {
    ...
    if (tableLines.length > 2 && index !== 0 && index !== 2) {
      ...
      for (const line of tableLines.slice(2)) {
        ...
        if (cells.every(s => /^[:\s-]+$/u.test(s))) continue
        cards.push({ title: `${heading} / ${cells[0]}`, body: columns.map((c, i) => `${c}：${cells[i] ?? ""}`).join("\n"), kind })
      }
    }
  }
  if (index >= 3 && Array.isArray(view.model?.modelElements)) {
    for (const m of view.model.modelElements) cards.push({ title: `${m.id} ${m.name ?? ""} · 模型合同记录`,
      body: [m.type, m.responsibility, ...(m.productionPaths ?? []), ...(m.testPaths ?? [])].filter(Boolean).join("\n"), kind: "model" })
    for (const inv of view.model.invariants ?? []) cards.push({ title: inv.id, body: inv.statement ?? "", kind: "model" })
  }
  if (index >= 4 && Array.isArray(view.plan?.slices)) {
    for (const s of view.plan.slices) {
      ...
      cards.push({ title: `${done ? "✓ 已实施" : "○ 计划"} ${s.id} ${s.title}`, kind: "evidence", body:
        `${(s.dependsOn ?? []).join(" + ") || "起点"} ──▶ ${s.id}\n业务结果：${s.outcome}\n消费者：${s.consumer}\n验收：${(s.acceptanceCriteria ?? []).join("；")}\n测试：${(s.testPaths ?? []).join("；")}\n回滚：${(s.rollback?.steps ?? []).join("；")}`,
        edges: (s.dependsOn ?? []).map((dep: string) => ({ from: dep, to: s.id, label: "依赖", candidate: false })) })
    }
  }
  if (!cards.length) cards.push({ title: "历史产物", body: "该里程碑缺少可识别的结构化章节，请切换到「正文」阅读原始结论。未推测业务关系。", kind: "warning" })
  return cards
}
