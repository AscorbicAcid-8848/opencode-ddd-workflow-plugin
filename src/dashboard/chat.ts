import { type WorkflowItem, milestoneStatus, romans } from "./model.js"
import { getLinkedSession } from "./session-links.js"
import type { TuiPluginApi } from "@opencode-ai/plugin/tui"

export function progressState(item: WorkflowItem, roman: string): string {
  if (milestoneStatus(item, roman) === "已批准") return "已完成"
  const first = romans.find(r => milestoneStatus(item, r) !== "已批准")
  if (roman !== first) return ""
...
export async function milestoneChat(item: WorkflowItem, roman: string, client: TuiPluginApi["client"]) {
  const windows = milestoneWindows(item, roman)
  if (!windows.length) return { messages: [], missing: true, warnings: ["没有可用的里程碑时间范围"] }
  const messages: Array<{ id: string; sessionID: string; role: string; text: string; time: number }> = []
  const warnings: string[] = []
  const sessions = [...new Set([item.state?.runtimeSessionId, ...(item.state?.runtimeSessionIds ?? [])].filter((id): id is string => !!id))]
  for (const sessionID of sessions) {
    try {
    await getLinkedSession(item, sessionID, client)
    const all: any[] = []; const cursors = new Set<string>(); let before: string | undefined
    for (;;) {
      const result = await client.session.messages({ directory: item.project, sessionID, limit: 100, before })
      if (result.error || !result.data) throw new Error("无法读取会话聊天记录")
      const page = result.data
      all.push(...page)
      if (page.length < 100) break
      const oldest = [...page].sort((a,b) => a.info.time.created - b.info.time.created)[0]?.info.id
      if (!oldest || cursors.has(oldest)) throw new Error("会话分页未前进")
      cursors.add(oldest); before = oldest
    }
    const insideWindow = (m: any) => windows.some(w => m.info.time.created >= w.start && m.info.time.created < w.end)
    ...
    for (const m of all) {
      if (seen.has(m.info.id) || !["user", "assistant"].includes(m.info.role)
        || !(insideWindow(m) || (m.info.role === "user" && parentPrompts.has(m.info.id)))) continue
      seen.add(m.info.id)
      const text = m.parts.filter((p: any) => p.type === "text" && !p.ignored).map((p: any) => p.text).join("\n")
      if (text) messages.push({ id: m.info.id, sessionID, role: m.info.role, text, time: m.info.time.created })
    }
    } catch (error) { warnings.push(`${sessionID}: ${(error as Error).message}`) }
  }
...
export function milestoneWindows(item: WorkflowItem, roman: string) {
  const state = item.state
  if (!state) return []
  const events: Array<{ time: number; roman: string; priority: number }> = []
  const add = (time: string | undefined, r: string | undefined, priority: number) => {
    const timestamp = Date.parse(time ?? "")
    if (Number.isFinite(timestamp) && r) events.push({ time: timestamp, roman: r, priority })
  }
  ...
  for (const c of state.checkpoints) {
    ...
    if (review) {
      const index = romans.indexOf(c.milestone as typeof romans[number])
  ...
  if (!Number.isFinite(end) && end !== Infinity) return []
