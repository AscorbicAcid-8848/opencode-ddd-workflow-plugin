...
interface Preferences { key: string; milestone: number; query: string; status: string; type: string }
const defaults: Preferences = { key: "", milestone: 0, query: "", status: "全部", type: "全部" }

/** The native route is independently exportable for real OpenTUI render tests. */
export function createDashboard(api: TuiPluginApi, project: string, leave: () => void, originSessionID?: string) {
  ...
  let chatResult: Awaited<ReturnType<typeof milestoneChat>> | undefined
  ...
  const notify = (message: string, variant: "error" | "success" | "info" = "info") => api.ui.toast({ message: t(message), variant })
  ...
  function button(parent: BoxRenderable, label: string, action: () => void, fg = theme.text) {
    ...
    const b = new BoxRenderable(api.renderer, { id: `ddd-button-${label}`, border: true, borderColor: fg, paddingX: 1, flexShrink: 0,
      onMouseDown: event => {
        if (event.button !== MouseButton.LEFT || busy || api.ui.dialog.open) return
        pressed = true
      ...
      onMouseUp: event => {
        ...
        if (activate) queueMicrotask(() => { if (!disposed && !busy && !api.ui.dialog.open) action() })
      } })
  ...
  function choose(title: string, options: Array<{ title: string; value: string; description?: string; disabled?: boolean }>, onSelect: (value: string) => void) {
    api.ui.dialog.replace(() => createComponent(api.ui.DialogSelect<string>, { title: t(title), options, onSelect: option => {
      api.ui.dialog.clear(); onSelect(option.value); if (!api.ui.dialog.open) root.focus()
    } }))
  }
  function search() {
  ...
  async function openSessions() {
    ...
    try {
      ...
      if (!item) throw new Error(t("工作流已移动，请刷新"))
      const sessions = await listLinkedSessions(item, api.client)
      if (disposed) return
      if (!sessions.length) { notify("此工作流尚未记录关联会话。旧版未保存的历史无法自动恢复。", "info"); return }
      choose("选择关联会话", sessions.map(s => ({ value: s.id, title: `${s.title}${s.current ? ` · ${t("当前执行会话")}` : ""}${s.archived ? ` · ${t("归档")}` : ""}`,
        ...
        disabled: !!s.error })), sessionID => {
          ...
          void getLinkedSession(item, sessionID, api.client).then(() => {
            if (!disposed) api.route.navigate("session", { sessionID })
  ...
  function continueWorkflow() {
    if (busy || !selected) return
    const item = selected
    if (item.archived || item.legacy || item.issues.length || ["complete", "rejected"].includes(item.state?.status ?? "")) { notify("该工作流不可续跑，请刷新查看", "error"); return }
    const launch = (target: string | null) => {
      if (busy || disposed) return
      busy = true
      void resumeInSession(item, target, api.client).then(sessionID => {
        if (!disposed) api.route.navigate("session", { sessionID })
      }).catch(error => { if (!disposed) notify((error as Error).message, "error") })
    ...
    choose("在哪里继续工作流", [
      ...
      if (value === "current" && originSessionID) launch(originSessionID)
      else if (value === "last" && item.state?.runtimeSessionId) launch(item.state.runtimeSessionId)
      else if (value === "new") launch(null)
      else if (value === "other") {
        busy = true
        void projectSessions(project, api.client).then(sessions => {
          if (disposed) return
          if (!sessions.length) { notify("当前目录没有可用会话"); return }
          choose("选择续跑会话", sessions.map(s => ({ title: s.title || s.id, value: s.id, description: s.id })), launch)
        }).catch(error => notify((error as Error).message, "error")).finally(() => { busy = false })
      }
    })
  }
  ...
  async function select(item?: WorkflowItem) {
    ...
    if (chatKey !== nextChatKey) { chatOpen = false; chatGeneration++; chatResult = undefined; chatLoading = false; chatText = ""; chatKey = nextChatKey }
    selected = item; view = undefined
    const seq = ++generation
    clear(body); clear(controls)
    if (!item) { card("没有匹配的 Workflow", t("调整筛选；项目尚无流程时，可返回会话使用 /ddd <需求> 创建。")); return }
  ...
  async function loadChat() {
    if (!selected || !view || chatLoading) return
    const item = selected, roman = view.roman, key = chatKey, sequence = ++chatGeneration
    chatLoading = true; chatText = ""; renderDetail()
    try {
      const result = await milestoneChat(item, roman, api.client)
      if (!disposed && sequence === chatGeneration && chatKey === key) chatResult = result
    } catch (error) {
      if (!disposed && sequence === chatGeneration && chatKey === key) chatText = String(error)
    } finally {
      if (!disposed && sequence === chatGeneration && chatKey === key) { chatLoading = false; renderDetail() }
    }
  }

  function renderDetail() {
    if (!selected || !view || disposed) return
    clear(timeline); clear(body); clear(controls)
    ...
    button(body, chatOpen ? "▼ 收起聊天记录" : "▶ 展开聊天记录", () => {
      chatOpen = !chatOpen
      if (chatOpen && !chatResult && !chatLoading) void loadChat()
      else renderDetail()
      root.focus()
    })
    if (chatOpen) {
      button(body, "刷新聊天记录", () => { if (!chatLoading) void loadChat() })
      if (chatLoading) card("聊天记录", t("加载中"))
      else if (chatText) card("聊天记录", chatText, theme.error)
      if (chatResult) {
        card("聊天记录", t("按时间汇总关联会话，包含闲聊；不代表每条消息都针对本里程碑。"), theme.textMuted)
        for (const m of chatResult.messages) card(`${t(m.role === "user" ? "用户" : "助手")} · ${new Date(m.time).toLocaleString()} · ${m.sessionID}`, m.text, m.role === "user" ? theme.primary : theme.success)
        if (!chatResult.messages.length) card("聊天记录", t("没有可准确归属的聊天记录；旧会话仍可通过执行会话查看。"))
        for (const warning of chatResult.warnings) card("一致性提示", warning, theme.warning)
      }
    }
    ...
    if (selected.legacy) card("旧版记录 · 只读", t("已兼容显示旧版文档与人工审核证据；不会通过面板迁移或修改旧版工作流。"), theme.textMuted)
    if (!view.body && !view.issues.length) {
      if (view.status === "待执行") card("待执行", t("前置审核已通过，尚无本阶段产物。执行后将在这里显示。"), theme.warning)
      else card(view.status === "未开始" ? "尚未开始" : "正在生成", t(view.status === "未开始"
      ...
      if (view.checkpoint?.decisionItems?.length) card("本次请您确认", decisionText(view, language), theme.warning)
      ...
      if (view.checkpoint?.reviewChecklist?.length) card("验收清单", view.checkpoint.reviewChecklist.map(s => `□ ${s}`).join("\n"))
      if (view.tasks) card("交付任务", view.tasks)
    }
    if (showHistory) {
      for (const ref of view.revisions ?? []) button(body, `${t("查看投影版本")} ${ref.revision} · ${ref.status}`, () => {
      ...
      if (!view.history.length) card("历史", t("尚无阶段提交记录"))
    }
    ...
    } else if (selected.status === "待执行") button(controls, "执行下一阶段", continueWorkflow, theme.primary)
  ...
  async function syncReview(item: WorkflowItem, milestone: MilestoneView) {
    try {
      const sent = await sendPanelReview(item, milestone, api.client)
      notify("审核已同步，正在返回原会话。", "success")
      if (!disposed) api.route.navigate("session", { sessionID: sent.sessionID })
    } catch (error) {
  ...
  function startReview(decision: ReviewDecision) {
    ...
    function chooseNext(i: number) {
      if (i < open.length) {
        const d = open[i]
        choose(`${t("批准前确认")} ${i + 1}/${open.length}：${d.question}${d.recommendationId ? "" : t("（尚未标注推荐方案）")}`,
          d.options.map(o => ({ title: `${o.label}${o.id === d.recommendationId ? t("（推荐）") : ""}`, description: o.impact, value: o.id })),
          id => { selections[d.id] = id; chooseNext(i + 1) })
      } else if (decision === "approve") confirm(t("批准当前正式里程碑方案"))
      else api.ui.dialog.replace(() => createComponent(api.ui.DialogPrompt, { title: t("请填写具体反馈"), onConfirm: feedback => {
  ...
  async function refresh(force = false) {
    ...
    try {
      ...
      if (language !== nextLanguage) { language = nextLanguage; force = true }
  ...
  root.onKeyDown = event => {
    ...
    else if (event.name === "s") void openSessions()
    else if (event.name === "c") continueWorkflow()
  ...
  const unregisterKeys = api.keymap.registerLayer({ target: root, targetMode: "focus-within", priority: 100,
    bindings: ["up", "down", "left", "right", "pagedown", "pageup", "r", "/", "f", "t", "s", "c", "a", "e", "x", "escape"].map(key => ({ key,
      cmd: () => {
        if (api.ui.dialog.open || busy) return false
        root.onKeyDown?.({ name: key, preventDefault() {}, stopPropagation() {} } as any)
      } })) })
  const timer = setInterval(() => { void refresh() }, 2000)
  const dispose = () => { if (disposed) return; disposed = true; generation++; clearInterval(timer); unsubscribe(); unregisterKeys() }
...
export const tui: TuiPlugin = async (api) => {
  const t = (text: string) => translate(resolveLanguage(api.state.config), text)
  if (!api.route?.register || !api.keymap?.registerLayer || !api.ui?.dialog) {
    api.ui?.toast?.({ message: t("当前宿主不支持 DDD TUI 控制面板"), variant: "warning" }); return
  }
  ...
  const unregisterCommand = api.keymap.registerLayer({ commands: [{ name: "ddd.workflow.panel", title: t("DDD Workflow 控制面板"), desc: t("查看当前项目的流程与六个人工里程碑"), category: "DDD",
    namespace: "palette", slashName: ROUTE, run: () => { if (api.route.current.name !== ROUTE) previous = api.route.current; api.ui.dialog.clear(); api.route.navigate(ROUTE) } }] })
