...
export function normalizeStructuredPlan(raw: any, current?: StructuredDeliveryPlan): StructuredDeliveryPlan {
  ...
  const slices = Array.isArray(raw?.slices) ? raw.slices.map((slice: any) => {
    ...
    const scalar = (field: keyof DeliverySlice) => slice?.[field] === undefined ? text(prior?.[field]) : text(slice[field])
    const array = (field: keyof DeliverySlice) => slice?.[field] === undefined ? [...((prior?.[field] as string[] | undefined) ?? [])] : list(slice[field])
    return {
...
export function validateStructuredPlan(plan: StructuredDeliveryPlan): PlanFinding[] {
  const findings: PlanFinding[] = []
  const required = (value: string, path: string, label: string) => {
    if (!value) findings.push({ code: "PLAN_FIELD_REQUIRED", path, message: `${label}不能为空。` })
  }
  required(plan.title, "plan.title", "计划标题")
  required(plan.objective, "plan.objective", "业务目标")
  if (!plan.capabilities.length) findings.push({ code: "PLAN_CAPABILITY_REQUIRED", path: "plan.capabilities", message: "新增功能至少需要一个行为 capability。" })
  if (!plan.slices.length) findings.push({ code: "PLAN_SLICE_REQUIRED", path: "plan.slices", message: "至少需要一个可独立验收的纵向切片。" })

  const capabilityIds = new Set<string>()
  plan.capabilities.forEach((capability, ci) => {
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(capability.id)) findings.push({ code: "PLAN_CAPABILITY_ID_INVALID", path: `plan.capabilities[${ci}].id`, message: "capability id 必须是 kebab-case。" })
    if (capabilityIds.has(capability.id)) findings.push({ code: "PLAN_CAPABILITY_DUPLICATE", path: `plan.capabilities[${ci}].id`, message: `capability ${capability.id} 重复。` })
    capabilityIds.add(capability.id)
    if (!capability.requirements.length) findings.push({ code: "PLAN_REQUIREMENT_REQUIRED", path: `plan.capabilities[${ci}].requirements`, message: "每个 capability 至少需要一个 Requirement。" })
    capability.requirements.forEach((requirement, ri) => {
      ...
      if (!requirement.scenarios.length) findings.push({ code: "PLAN_SCENARIO_REQUIRED", path: `plan.capabilities[${ci}].requirements[${ri}].scenarios`, message: "每个 Requirement 至少需要一个 Given/When/Then Scenario。" })
  ...
  plan.slices.forEach((slice, index) => {
    if (!/^[A-Za-z][A-Za-z0-9_-]*$/u.test(slice.id)) findings.push({ code: "PLAN_SLICE_ID_INVALID", path: `plan.slices[${index}].id`, message: "切片 ID 必须稳定且以字母开头。" })
    if (sliceIds.has(slice.id)) findings.push({ code: "PLAN_SLICE_DUPLICATE", path: `plan.slices[${index}].id`, message: `切片 ${slice.id} 重复。` })
    sliceIds.add(slice.id)
    for (const [field, label] of [["title", "标题"], ["outcome", "可观察结果"], ["consumer", "真实消费者"], ["compatibility", "兼容策略"], ["rollback", "回滚策略"]] as const) required(slice[field], `plan.slices[${index}].${field}`, `切片${label}`)
    for (const [field, label] of [["acceptanceCriteria", "验收标准"], ["modelElementIds", "ME 模型元素"], ["invariantIds", "INV 不变量"], ["productionPaths", "生产路径"], ["testPaths", "测试路径"], ["verification", "验证命令"]] as const) {
      if (!slice[field].length) findings.push({ code: "PLAN_SLICE_LIST_REQUIRED", path: `plan.slices[${index}].${field}`, message: `切片必须声明${label}。` })
    }
  })
  plan.slices.forEach((slice, index) => slice.dependsOn.forEach((dependency) => {
    if (!sliceIds.has(dependency)) findings.push({ code: "PLAN_DEPENDENCY_UNKNOWN", path: `plan.slices[${index}].dependsOn`, message: `依赖切片 ${dependency} 不存在。` })
    if (dependency === slice.id) findings.push({ code: "PLAN_DEPENDENCY_SELF", path: `plan.slices[${index}].dependsOn`, message: "切片不能依赖自身。" })
  }))
  ...
  if ([...sliceIds].some(cycle)) findings.push({ code: "PLAN_DEPENDENCY_CYCLE", path: "plan.slices", message: "纵向切片依赖图必须无环。" })
  return findings
}

export function compileStructuredPlan(plan: StructuredDeliveryPlan, workflowId: string) {
...
export function compileDeliveryMilestoneSections(
  ...
  const requirementRows = plan.capabilities.flatMap((capability) => capability.requirements.flatMap((requirement) =>
    requirement.scenarios.map((scenario) => `- ${capability.id} / ${requirement.name} / ${scenario.name}：WHEN ${scenario.when}；THEN ${scenario.then}`)))
