...
const ENGLISH_QUERY_RESULT = new RegExp(
  `\\b[A-Za-z0-9]*${ENGLISH_QUERY_SUBJECT}[A-Za-z0-9]*(?:Returned|Queried|Loaded|Displayed|Retrieved)\\b`,
  "u",
)
...
export const BUSINESS_RULE_FAMILIES: BusinessRuleFamily[] = [
  {
...
export function deferredRuleFamilies(document: string): string[] {
...
export function requestedInvariantClauses(text: string, kind: InvariantClauseKind): string[] {
...
function semanticAnchors(text: string): string[] {
...
export function invariantCoversClause(domainText: string, clause: string): boolean {
  if (!/\bINV-\d+\b/u.test(domainText)) return false
...
export function requestedTermDistinctions(text: string): TermDistinction[] {
  const result: TermDistinction[] = []
...
export function textCoversDistinction(text: string, distinction: TermDistinction): boolean {
  if (!text.includes(distinction.left) || !text.includes(distinction.right)) return false
