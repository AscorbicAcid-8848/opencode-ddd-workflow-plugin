...
export async function withWorkflowLock<T>(
  ...
  while (!handle) {
    try {
      ...
      if (Date.now() - started >= timeoutMs) throw new WorkflowLockError(lockPath)
      await sleep(retryMs)
    }
  }
  try {
...
export async function readWorkflowLock(workflowRoot: string): Promise<Record<string, unknown> | null> {
  const lockPath = path.join(workflowRoot, ".ddd", "workflow.lock")
  try { return JSON.parse(await readFile(lockPath, "utf8")) as Record<string, unknown> } catch { return null }
}
