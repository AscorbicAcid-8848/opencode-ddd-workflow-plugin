...
function section(item: any): string {
  return [`## ${item.heading}`, ...item.subsections.map((heading: string) => `### ${heading}\n\n- 待本里程碑补充。`)].join("\n\n")
}

export async function documentTemplate(title: string, key: string): Promise<string> {
  const catalog = await documents()
  const contract = catalog.documents[key]
  if (!contract) throw new WorkflowError(`Unknown milestone document contract: ${key}`)
...
export async function ensureDocumentSet(root: string, profile: WorkflowProfile, title: string): Promise<void> {
  for (const milestone of profile.milestones) {
    ...
    if (!await exists(file)) await atomicText(file, await documentTemplate(title, milestone.document))
  }
}

export function stageMarker(stageId: string) { return `<!-- ddd-stage:${stageId} -->` }
export function scopeMarker(scopeId: string) { return `<!-- ddd-scope:${scopeId} -->` }

export function addHiddenStageMetadata(content: string, stage: StageContract): string {
...
export async function validateFixedStructure(content: string, documentKey: string): Promise<void> {
  const catalog = await documents()
  const contract = catalog.documents[documentKey]
  const expected = [
    ...catalog.commonOverview.flatMap((item: any) => [item.heading, ...item.subsections]),
    ...contract.sections.flatMap((item: any) => [item.heading, ...item.subsections]),
    "业务验收记录",
  ]
  ...
  for (const token of [OVERVIEW_START, OVERVIEW_END, DETAIL_START, DETAIL_END, REVIEW_LOG, STAGE_MARKERS_START, STAGE_MARKERS_END]) {
    if (!content.includes(token)) throw new WorkflowError(`${documentKey} 缺少固定结构标记：${token}`)
  }
}

export function topLevelSections(content: string): Record<string, string> {
...
const comparable = (value: string) => value.replace(/<!--\s*ddd-.*?-->/gs, "").trim()

export async function changedSections(formalFile: string, candidateFile: string): Promise<Record<string, string>> {
...
export function replaceSubsection(content: string, heading: string, body: string): string {
  const expression = new RegExp(`(^### ${escapeRegExp(heading)}[ \\t]*$)([\\s\\S]*?)(?=^### |^## |${escapeRegExp(OVERVIEW_END)}|${escapeRegExp(DETAIL_END)}|$(?![\\s\\S]))`, "m")
  if (!expression.test(content)) throw new WorkflowError(`阶段文档缺少固定小节：${heading}`)
  return content.replace(expression, `$1\n\n${body.trim()}\n\n`)
}

export function replaceTopLevelSection(content: string, heading: string, body: string): string {
  const expression = new RegExp(`(^## ${escapeRegExp(heading)}[ \\t]*$)([\\s\\S]*?)(?=^## |$(?![\\s\\S]))`, "m")
  if (!expression.test(content)) throw new WorkflowError(`Stage document is missing fixed section: ${heading}`)
  return content.replace(expression, `$1\n\n${body.trim()}\n\n`)
}

export function validateHumanOverview(content: string): void {
...
export function validateHumanMilestoneDocument(content: string): void {
...
export function subsectionBody(content: string, heading: string): string {
...
export function appendReview(content: string, stage: StageContract, values: { time: string; decision: string; reviewer: string; feedback: string }): string {
  if (!content.includes(REVIEW_LOG)) throw new WorkflowError("阶段文档缺少业务验收记录区")
  const entry = `\n- **${stageTitle(stage)}**\n  - 时间：\`${values.time}\`\n  - 决定：${values.decision}\n  - 验收人：${values.reviewer}\n  - 反馈：${values.feedback}\n`
...
export async function readDocument(root: string, profile: WorkflowProfile, stage: StageContract): Promise<string> {
  return readFile(documentPath(root, profile, stage), "utf8")
}

export async function writeDocument(root: string, profile: WorkflowProfile, stage: StageContract, content: string): Promise<void> {
  await atomicText(documentPath(root, profile, stage), content)
}

export const documentName = (root: string, profile: WorkflowProfile, stage: StageContract) => path.basename(documentPath(root, profile, stage))
