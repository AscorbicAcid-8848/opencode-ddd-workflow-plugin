...
function milestoneFor(profile: WorkflowProfile, document: string | undefined) {
  return profile.milestones.find((item) => item.document === document) ?? null
}

function nextGate(profile: WorkflowProfile, after: number) {
...
export function workflowTransition(profile: WorkflowProfile, state: WorkflowState): Transition {
  ...
  const common = {
    schemaVersion: "ddd-workflow-transition/v1" as const,
  ...
  if (state.status === "revision_requested") return {
  ...
  if (state.status === "awaiting_archive") return {
