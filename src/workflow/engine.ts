...
import { addHiddenStageMetadata, appendReview, documentName, ensureDocumentSet, replaceSubsection, validateFixedStructure, validateHumanMilestoneDocument } from "./documents.js"
import { atomicBytes, atomicText, exists, fileEvidence, now, readJson, sha256, snapshot, writeJson } from "./fs.js"
import { action as openSpecAction, archive as archiveOpenSpec, ensureChange, loadLink, updateStatus, validatePlanning, validateStrict } from "./openspec.js"
...
export interface InitInput extends WorkflowIdentity { title: string; request: string }
export interface CheckpointInput extends WorkflowIdentity { stage: string; summary: string; evidenceFile?: string }
...
async function reconcile(root: string, state: WorkflowState): Promise<WorkflowState> {
  ...
  if (path.resolve(root) === path.resolve(legacy) && state.status !== "complete") throw new WorkflowError("该工作流仍位于旧 docs/ddd 布局；请先执行 migrate-layout")
  if (!await exists(openSpecLinkPath(root)) && state.status !== "complete") await ensureChange(root, state, `升级既有 DDD 工作流：${state.title}`)
  ...
  for (const checkpoint of state.checkpoints) {
    ...
    if (!contract.humanGate && checkpoint.reviewStatus === "awaiting_review") {
      checkpoint.reviewStatus = "not_required"; checkpoint.reviewPacket = null; changed = true
    }
...
export async function beginStage(input: WorkflowIdentity & { stage: string }) {
  await assertIdentity(input)
  ...
  if (!transition.allowedNextStages.includes(input.stage)) throw new WorkflowError(`当前状态不允许开始 ${input.stage}；合法后续阶段：${transition.allowedNextStages.join("、") || "无"}`)
  const stage = stageContract(profile, input.stage)
  const result = await initializeWorkbench(root, state, profile, stage)
  return { ...result, transition }
}

export async function prepareStage(input: WorkflowIdentity & { stage: string }) {
  await assertIdentity(input)
  const root = await workflowRoot(input)
  const state = await reconcile(root, await loadState(root))
  const profile = await profileFor(input.workflowType)
  const transition = workflowTransition(profile, state)
  if (!transition.allowedNextStages.includes(input.stage)) throw new WorkflowError(`Stage ${input.stage} is not allowed; allowed stages: ${transition.allowedNextStages.join(", ") || "none"}`)
  const stage = stageContract(profile, input.stage)
  const prepared = await prepareStageContract(root, state, profile, stage)
  return { artifactRoot: root, ...prepared, transition }
}

async function prepareStageContract(root: string, state: WorkflowState, profile: WorkflowProfile, stage: StageContract) {
  await initializeWorkbench(root, state, profile, stage)
  const bundle = stageBundle(root, profile, stage)
  const attempt = ((state.stageAttempts ?? {}) as Record<string, StageAttempt>)[stage.id]
  const savedDraft = await exists(bundle.draft) ? await readJson<StageSubmission>(bundle.draft) : null
...
function milestoneBatchStages(profile: WorkflowProfile, state: WorkflowState): StageContract[] {
  const transition = workflowTransition(profile, state)
  if (transition.stopAllowed || transition.requiredAction === "archive" || transition.requiredAction === "complete") return []
  ...
  for (const stage of profile.stages.slice(start)) {
    if ((stage.repeatable || stage.cycleGroup) && !stage.humanGate) {
      if (!stages.length) throw new WorkflowError("当前阶段属于可重复实现循环；请使用 prepare_stage/submit_stage 保持逐切片验证与 Git 提交")
      break
    }
    stages.push(stage)
    if (stage.humanGate) break
  }
  if (!stages.length || !stages.at(-1)?.humanGate) throw new WorkflowError("当前线性路径在下一个人工里程碑前包含实现循环；请使用单阶段工具")
  return stages
}

export async function prepareMilestone(input: WorkflowIdentity) {
  await assertIdentity(input)
  const root = await workflowRoot(input)
  const state = await reconcile(root, await loadState(root))
  const profile = await profileFor(input.workflowType)
  ...
  const prepared: any[] = []
  for (const stage of stages) prepared.push({ stage: stage.id, ...(await prepareStageContract(root, state, profile, stage)) })
...
async function submitMilestoneUnlocked(input: MilestoneSubmissionInput) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new WorkflowError(`里程碑批次必须按顺序完整提交：${expected.join("、")}`)
...
async function submitStageUnlocked(input: StageSubmissionInput) {
  ...
  const previousAttempt = (state.stageAttempts as Record<string, StageAttempt> | undefined)?.[stage.id]
  ...
  try {
    if (input.submission && input.repairPatch?.length) throw new Error("provide either submission or repair_patch, not both")
    if (input.repairPatch?.length) {
      if (!await exists(bundle.draft)) throw new Error("no saved submission draft exists for repair_patch")
      submission = applyStageSubmissionPatch(await readJson<StageSubmission>(bundle.draft), input.repairPatch)
    } else if (input.submission) submission = input.submission
    else throw new Error("submission is required for the first attempt; use repair_patch only after a failed attempt")
  } catch (error) {
  ...
  if (preflight.findings.length || !preflight.submission) return recordStageFailure(root, state, profile, stage, preflight.findings, bundle.draft)
  try {
    await compileStageSubmission(root, state, profile, stage, preflight.submission)
    if (stage.strategicBaselineGate) {
      await compileStrategicBaseline(
        root,
        state,
        stage.strategicBaselineGate as "inventory" | "decision-delta",
        preflight.submission.strategicBaseline!,
      )
    }
    ...
    const attempts = (successfulState.stageAttempts ?? {}) as Record<string, StageAttempt>
    delete attempts[stage.id]
    successfulState.stageAttempts = attempts
    delete successfulState.runtimeBlockedStage
    if (successfulState.status === "runtime_blocked") successfulState.status = "active"
...
async function recordCompilerFailure(root: string, state: WorkflowState, profile: WorkflowProfile, stage: StageContract, message: string, draft: string) {
  ...
  const attempts = (state.stageAttempts ?? {}) as Record<string, StageAttempt>
...
async function recordStageFailure(root: string, state: WorkflowState, profile: WorkflowProfile, stage: StageContract, findings: ValidationFinding[], draft?: string) {
  ...
  const progress: StageAttempt["progress"] = previousBest === undefined ? "first-failure"
  ...
  attempts[stage.id] = attempt
  state.stageAttempts = attempts
  if (blocked) { state.status = "runtime_blocked"; state.runtimeBlockedStage = stage.id }
  state.updatedAt = now()
  await saveState(root, state)
  await writePortal(root, state, profile)
  const transition = workflowTransition(profile, state)
...
async function submit(root: string, state: WorkflowState, stageId: string, summary: string, evidenceFile?: string): Promise<any> {
  ...
  if (state.status === "rejected") throw new WorkflowError("该工作流已被人工拒绝，不能重新启动")
  ...
  if (!bootstrapAllowed && !transitionBefore.allowedNextStages.includes(stageId)) throw new WorkflowError(`checkpoint 未经合法 transition 授权：${stageId}`)
  if (!summary.trim() || summary.trim().length < 8) throw new WorkflowError("summary 必须描述本阶段真实业务增量")
  if (state.checkpoints.some((item) => item.reviewStatus === "awaiting_review")) throw new WorkflowError("存在待人工验收的里程碑，不能继续提交")
  ...
  if (previous) {
    ...
    if (currentIndex < previousIndex && stage.cycleGroup === previousStage.cycleGroup && previous.reviewStatus === "approved") {
      for (const item of state.checkpoints) if (stageIndex(profile, item.stage) > currentIndex && item.reviewStatus === "approved") item.reviewStatus = "superseded"
    }
  }
  const bundle = await validateStageBundle(root, state, profile, stage)
  if (stage.humanGate) validateHumanMilestoneDocument(await readFile(bundle.candidate, "utf8"))
  ...
  if (expected && await exists(formal) && expected.sha256 !== await sha256(formal)) throw new WorkflowError(`正式里程碑文档 ${path.basename(formal)} 被绕过 workbench 直接修改`)

  let openSpecValidation: unknown
  let deliveryValidation: unknown
  let strategicBaseline: unknown
  if (stage.strategicBaselineGate) strategicBaseline = await validateStrategicBaseline(root, state, stage.strategicBaselineGate as "inventory" | "decision-delta")
  ...
  if (stage.implementationEvidence) implementation = await validateImplementationEvidence(root, state, stageId, evidenceFile)
  else if (evidenceFile) throw new WorkflowError("--evidence-file 只能用于实现阶段")
  if (stage.requiresCompletedImplementation) requireCompletedImplementation(state)
  ...
  const record: Checkpoint & { transition?: unknown } = {
  ...
  if (openSpecValidation) record.openSpec = openSpecValidation
  if (deliveryValidation) record.delivery = deliveryValidation
  if (strategicBaseline) {
    record.strategicBaseline = strategicBaseline
    state.strategicBaseline = strategicBaseline as Record<string, unknown>
  }
  if (implementation) record.implementationEvidence = implementation
  state.checkpoints.push(record)
  state.currentStage = stageId
  state.status = stage.humanGate ? "awaiting_review" : "active"
  state.updatedAt = now(); state.snapshot = currentSnapshot
  const transition = workflowTransition(profile, state)
  await saveState(root, state); await appendIncrement(root, { ...record, transition }); await writePortal(root, state, profile)
...
async function reviewUnlocked(input: ReviewInput) {
  ...
  if (!target) throw new WorkflowError(`No pending review exists for stage: ${input.stage}`)
  const stage = stageContract(profile, input.stage)
  if (!stage.humanGate) throw new WorkflowError("当前阶段不是人工验收卡点")
  const feedback = input.feedback?.trim() || "无补充意见。"
  if (["revise", "reject"].includes(input.decision) && !input.feedback?.trim()) throw new WorkflowError(`${input.decision} requires actionable feedback`)
  ...
  } else if (input.decision === "revise") await updateStatus(root, state, "revision-requested")
  else if (input.decision === "reject") await updateStatus(root, state, "rejected")
  ...
  state.updatedAt = reviewedAt; state.snapshot = await snapshot(root)
  await saveState(root, state); await appendReviewLog(root, target); await writePortal(root, state, profile)
  if (finalApproval) {
...
async function retryArchiveUnlocked(input: WorkflowIdentity) {
  ...
  const root = await workflowRoot(input)
  const state = await reconcile(root, await loadState(root))
  if (state.status === "complete") return status(input)
  if (state.status !== "awaiting_archive") throw new WorkflowError("只有最终验收已批准且待归档的工作流可以执行 archive")
  const profile = await profileFor(input.workflowType)
  const archivedRoot = await archiveOpenSpec(root, state)
  await saveState(archivedRoot, state)
  await writePortal(archivedRoot, state, profile)
...
export async function getOpenSpecAction(input: WorkflowIdentity & { artifact: OpenSpecArtifact }) {
  const root = await workflowRoot(input)
  const state = await reconcile(root, await loadState(root))
  const profile = await profileFor(state.workflowType)
  const planning = new Set<OpenSpecArtifact>(["proposal", "specs", "design", "tasks"])
  if (planning.has(input.artifact) && !state.checkpoints.some((item) => item.criticalGate === "tactical-design" && item.reviewStatus === "approved")) throw new WorkflowError("OpenSpec planning artifacts 只能在里程碑 IV 批准后生成")
  if (input.artifact === "apply") {
    if (!state.checkpoints.some((item) => stageContract(profile, item.stage).openspecArtifactGate && item.reviewStatus === "approved")) throw new WorkflowError("OpenSpec apply instructions 只能在里程碑 V 批准后获取")
    await validatePlanning(root, state, "plan")
  }
  return openSpecAction(root, state, input.artifact)
}

export async function migrateLayout(input: MigrateInput) {
  await assertIdentity(input)
  const source = path.resolve(input.legacyRoot)
  const target = await canonicalRoot(input)
  if (await exists(target) || (await archiveCandidates(input.projectRoot, input.workflowId)).length) throw new WorkflowError(`目标工作流已存在或 change 已归档：${target}`)
  ...
  if (!await exists(sourceStateFile)) throw new WorkflowError(`Legacy workflow state is missing under: ${source}`)
  const state = await readJson<WorkflowState>(sourceStateFile)
  if (state.workflowType !== input.workflowType || state.workflowId !== input.workflowId) throw new WorkflowError("Legacy workflow identity does not match migration request")
  const profile = await profileFor(input.workflowType)
  state.projectRoot = path.resolve(input.projectRoot); state.artifactRoot = target; state.schemaVersion = WORKFLOW_SCHEMA
  state.profileSchemaVersion = (await profiles()).schemaVersion; state.documentLayoutVersion ??= LAYOUT_SCHEMA
  state.migratedFrom = source; state.migratedAt = now(); state.updatedAt = now()
  ...
  for (const milestone of profile.milestones) {
    ...
    if (old) await cp(old, path.join(target, name), { force: true })
  }
  ...
  for (const name of ["checkpoints", "delivery", "implementation-evidence", "increment-log.md"]) {
    ...
    if (await exists(old)) await cp(old, path.join(targetInternal, name), { recursive: true, force: true })
  }
...
async function initializeWorkbench(root: string, state: WorkflowState, profile: WorkflowProfile, stage: StageContract) {
  const bundle = stageBundle(root, profile, stage)
  await mkdir(bundle.workbench, { recursive: true })
  ...
  await writeJson(bundle.output, output); await writeJson(bundle.review, review)
  return {
...
async function bootstrap(root: string, state: WorkflowState, profile: WorkflowProfile, stage: StageContract) {
  const result = await initializeWorkbench(root, state, profile, stage)
  ...
  const output = await readJson<any>(result.stageOutput)
  ...
  output.soleOutput.statement = `后续仅执行 ${state.workflowType} 工作流`; output.soleOutput.itemRefs = ["ROUTE-001"]
  await writeJson(result.stageOutput, output)
  const review = await readJson<any>(result.scopeReview)
  review.candidateDocumentSha256 = await sha256(result.candidateDocument)
  review.stageOutputSha256 = await sha256(result.stageOutput); review.reviewedAt = now()
...
function requireCompletedImplementation(state: WorkflowState) {
  const implementation = state.checkpoints.filter((item) => /implementation/.test(item.stage))
  if (!implementation.length || implementation.every((item) => !item.implementationEvidence)) throw new WorkflowError("最终验收前必须存在通过验证的实现证据")
}

function renderDelta(stage: string, before: WorkflowState["snapshot"], after: WorkflowState["snapshot"]) {
  const rows: string[] = []
  for (const file of [...new Set([...Object.keys(before), ...Object.keys(after)])].sort()) {
    if (!before[file]) rows.push(`| added | \`${file}\` | \`${after[file].sha256}\` |`)
    else if (!after[file]) rows.push(`| removed | \`${file}\` | - |`)
    else if (before[file].sha256 !== after[file].sha256) rows.push(`| modified | \`${file}\` | \`${after[file].sha256}\` |`)
  }
  return `# ${stage} 文件增量\n\n| 变化 | 文件 | SHA-256 |\n|---|---|---|\n${rows.join("\n") || "| unchanged | - | - |"}\n`
}

async function writePortal(root: string, state: WorkflowState, profile: WorkflowProfile) {
...
async function appendIncrement(root: string, value: any) {
...
async function appendReviewLog(root: string, checkpoint: Checkpoint) {
  const file = path.join(internalRoot(root), "increment-log.md")
  const current = await exists(file) ? await readFile(file, "utf8") : "# DDD 增量日志\n"
