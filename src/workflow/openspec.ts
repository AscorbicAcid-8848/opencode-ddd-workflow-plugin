...
function isNodeExecutable(executable: string | undefined): boolean {
...
export async function runOpenSpec(projectRoot: string, args: string[]): Promise<string> {
  ...
  try {
    ...
    throw new WorkflowRuntimeError(
      "OPENSPEC_LAUNCH_FAILED",
      "openspec-cli",
      `无法用 Node 运行内置 OpenSpec CLI（node=${node}，host=${process.execPath}）：${(error as Error).message}`,
    )
  }
}
...
export async function runOpenSpecJson(projectRoot: string, args: string[]): Promise<any> {
  ...
  try { return JSON.parse(output) } catch {
    throw new WorkflowRuntimeError(
      ...
      "openspec-cli",
      `内置 OpenSpec CLI 没有返回有效 JSON：${args.join(" ")}；输出：${output}`,
    )
  }
}

async function ensureProject(projectRoot: string) {
  ...
  if (!await exists(config)) await atomicText(config, "schema: spec-driven\n\ncontext: |\n  每个 DDD workflow-id 对应一个 OpenSpec change；六份 DDD 里程碑、工程计划和证据由同一个 change 托管。\n")
  return root
}

async function writeTrace(root: string, state: WorkflowState, status: string, change: string, archivedAt?: string) {
  const trace: any = {
  ...
  if (archivedAt) trace.archivedAt = archivedAt
...
export async function ensureChange(root: string, state: WorkflowState, request: string) {
  ...
  if (archives.length) throw new WorkflowError(`OpenSpec change 已归档，不能复用 workflow-id：${archives.at(-1)}`)
  let creation: any = null
  if (!await exists(change)) creation = await runOpenSpecJson(state.projectRoot, ["new", "change", state.workflowId, "--schema", "spec-driven", "--json"])
  if (!await exists(path.join(change, ".openspec.yaml"))) throw new WorkflowError(`OpenSpec new change 未生成标准骨架：${change}`)
  const readme = path.join(change, "README.md")
  if (!await exists(readme)) await atomicText(readme, `# ${state.title}\n\n- DDD 里程碑：\`${relative(state.projectRoot, root)}/README.md\`\n- 原始请求：${request.trim()}\n- 状态：等待 DDD 战术设计批准后生成 OpenSpec planning artifacts。\n`)
  const link: any = await writeTrace(root, state, "reserved", change)
  if (creation) link.creation = { engine: `@fission-ai/openspec@${OPENSPEC_VERSION}`, action: "new change", schema: creation.schemaName ?? "spec-driven" }
  state.openSpec = link
  await writeJson(openSpecLinkPath(root), link)
  return link
}

export async function loadLink(root: string): Promise<any> {
  if (!await exists(openSpecLinkPath(root))) throw new WorkflowError("当前 DDD 工作流缺少 OpenSpec 双向链接")
  const link = await readJson<any>(openSpecLinkPath(root))
  if (link.schema !== "ddd-openspec-link/v2") throw new WorkflowError(`Unsupported OpenSpec link schema: ${link.schema}`)
  return link
}

export async function updateStatus(root: string, state: WorkflowState, status: string) {
  const link = await loadLink(root)
  return writeTrace(root, state, status, path.join(state.projectRoot, link.changePath))
}

export async function status(state: WorkflowState): Promise<any> {
  const value = await runOpenSpecJson(state.projectRoot, ["status", "--change", state.workflowId, "--json"])
  if (value.changeName !== state.workflowId || value.schemaName !== "spec-driven") throw new WorkflowError("OpenSpec status 与当前 DDD change 不一致")
  return value
}

export async function action(root: string, state: WorkflowState, artifact: OpenSpecArtifact) {
  ...
  if (instructions.changeName !== state.workflowId || instructions.artifactId !== artifact) throw new WorkflowError("OpenSpec instructions 与当前请求不一致")
  const mapping: Record<OpenSpecArtifact, string[]> = {
  ...
  const dddInputs: string[] = []
  for (const file of mapping[artifact]) if (await exists(path.join(root, file))) dddInputs.push(file)
  return {
...
export async function validateStrict(state: WorkflowState) {
  const current = await status(state)
  const output = await runOpenSpec(state.projectRoot, ["validate", state.workflowId, "--type", "change", "--strict", "--json", "--no-interactive"])
  let validation: any
  try { validation = JSON.parse(output) } catch { validation = { status: "passed", output } }
  return { status: current, validation }
}

export async function validatePlanning(root: string, state: WorkflowState, phase: "plan" | "implementation" | "archive") {
  const link = await loadLink(root)
  ...
  for (const [name, file] of Object.entries(files)) if (!await exists(file)) missing.push(name)
  const specsRoot = path.join(change, "specs")
  const specs: string[] = []
  if (await exists(specsRoot)) for (const entry of await readdir(specsRoot, { withFileTypes: true })) {
    const file = path.join(specsRoot, entry.name, "spec.md")
    if (entry.isDirectory() && await exists(file)) specs.push(file)
  }
  const metadata = await readFile(path.join(change, ".openspec.yaml"), "utf8")
  const skipSpecs = /^skip_specs\s*:\s*true\s*$/im.test(metadata)
  if (!skipSpecs && !specs.length) missing.push("specs/<capability>/spec.md")
  if (skipSpecs && state.workflowType !== "refactor-system") throw new WorkflowError("只有不改变外部行为的重构可以 skip_specs")
  if (missing.length) throw new WorkflowError(`OpenSpec planning artifacts 不完整：${missing.join("、")}`)
  const proposal = await readFile(files.proposal, "utf8")
  for (const heading of ["## Why", "## What Changes", "## Capabilities", "## Impact"]) if (!proposal.includes(heading)) throw new WorkflowError(`OpenSpec proposal 缺少：${heading}`)
  const tasks = await readFile(files.tasks, "utf8")
  const design = await readFile(files.design, "utf8")
  for (const heading of ["## Context", "## Goals / Non-Goals", "## Decisions", "## Risks / Trade-offs"]) if (!design.includes(heading)) throw new WorkflowError(`OpenSpec design 缺少：${heading}`)
  let requirements = 0, scenarios = 0
  for (const file of specs) {
    const content = await readFile(file, "utf8")
    if (!/^## (?:ADDED|MODIFIED|REMOVED|RENAMED) Requirements\s*$/m.test(content)) throw new WorkflowError(`OpenSpec delta spec 缺少操作区：${file}`)
    ...
    if (!currentRequirements || currentScenarios < currentRequirements || !/\b(?:MUST|SHALL)\b/.test(content) || !/\*\*WHEN\*\*/.test(content) || !/\*\*THEN\*\*/.test(content)) throw new WorkflowError(`OpenSpec Requirement/Scenario 合同不完整：${file}`)
    requirements += currentRequirements; scenarios += currentScenarios
  }
  const checked = [...tasks.matchAll(/^- \[[xX]\] \d+\.\d+\s+.+$/gm)].length
  const unchecked = [...tasks.matchAll(/^- \[ \] \d+\.\d+\s+.+$/gm)].length
  if (!checked && !unchecked) throw new WorkflowError("OpenSpec tasks.md 没有可追踪任务")
  if (phase === "plan" && !unchecked) throw new WorkflowError("进入 Coding 前应有待实现任务")
  if (phase === "implementation" && !checked) throw new WorkflowError("实现检查点要求至少完成一个任务")
  if (phase === "archive" && unchecked) throw new WorkflowError(`OpenSpec 仍有 ${unchecked} 个未完成任务`)
  return { changeId: state.workflowId, phase, requirements, scenarios, tasksCompleted: checked, tasksRemaining: unchecked, skipSpecs }
}

export async function archive(root: string, state: WorkflowState) {
  ...
  if (!await exists(activeChange(state.projectRoot, state.workflowId)) && existingArchives.length) {
    ...
    if (!await exists(archivedRoot)) throw new WorkflowError("OpenSpec 已移动 change，但归档中缺少 DDD 包")
  ...
  if (await exists(activeChange(state.projectRoot, state.workflowId)) || !candidates.length) throw new WorkflowError("OpenSpec archive 后置条件失败")
  ...
  state.updatedAt = now()
  state.openSpec = await writeTrace(archivedRoot, state, "archived", archived, now())
  ;(state.openSpec as any).validation = strict
  return archivedRoot
}

async function writeHistory(projectRoot: string) {
  ...
  for (const item of await readdir(changes, { withFileTypes: true })) {
    if (!item.isDirectory() || item.name === "archive") continue
    const traceFile = path.join(changes, item.name, "ddd-workflow.json")
    const trace = await exists(traceFile) ? await readJson<any>(traceFile) : {}
  ...
  if (!rows.length) rows.push("| - | 暂无 change | - | - | - |")
