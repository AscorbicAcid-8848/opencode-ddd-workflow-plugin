...
export interface WorkflowState {
  ...
  snapshot: Record<string, { sha256: string; bytes: number }>
...
export interface StrategicBaselineSubmission {
  currentSpecs: Array<{ path: string; relevance: "relevant" | "not-relevant"; reason: string }>
  changes: Array<{ path: string; relevance: "relevant" | "not-relevant"; reason: string }>
  recoveredDecisions: Array<{ id: string; sourcePath: string; decision: string; reason: string }>
  unresolvedConflicts: string[]
  strategicDisposition: {
    status: "pending" | "proposed"
    reused: Array<{ baselineDecisionId: string; rationale: string }>
    changed: Array<{ baselineDecisionId: string; proposedDecision: string; reason: string; impact: string }>
    new: Array<{ id: string; proposedDecision: string; reason: string; impact: string }>
    conflicts: string[]
  }
}
