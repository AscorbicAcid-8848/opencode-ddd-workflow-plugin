...
function runtimeFailure(error: WorkflowRuntimeError) {
  return {
    accepted: false,
    error: {
...
async function executeTool(operation: string, action: () => Promise<unknown>): Promise<string> {
  void operation
  try {
    return output(await action())
  } catch (error) {
    if (error instanceof WorkflowRuntimeError) return output(runtimeFailure(error))
    throw error
  }
}

function projectRoot(args: { project_root?: string }, context: { worktree?: string; directory?: string }) {
  return path.resolve(args.project_root || context.worktree || context.directory || process.cwd())
}

function identity(args: { workflow_type: WorkflowType; workflow_id: string; project_root?: string }, context: { worktree?: string; directory?: string }) {
  return { workflowType: args.workflow_type, workflowId: args.workflow_id, projectRoot: projectRoot(args, context) }
}

export const dddWorkflowTools: Record<string, any> = {
    ...
    ddd_workflow_prepare: tool({
      ...
      async execute(args, context) {
        if (args.mode === "stage" && !args.stage) throw new Error("stage is required when mode=stage")
        if (args.mode === "milestone" && args.stage) throw new Error("stage must be omitted when mode=milestone")
    ...
    ddd_workflow_submit: tool({
      ...
      async execute(args, context) {
        if (args.mode === "stage" && args.submissions.length !== 1) throw new Error("mode=stage requires exactly one submission entry")
        const submissions = args.submissions.map((entry) => ({
          stage: entry.stage,
          summary: entry.summary,
          submission: entry.submission as any,
          repairPatch: entry.repair_patch as any,
...
export const DddWorkflowProtectionPlugin: Plugin = async (pluginInput) => ({
  async config(config) {
    injectMilestoneEditProtection(config as Record<string, any>)
  },
