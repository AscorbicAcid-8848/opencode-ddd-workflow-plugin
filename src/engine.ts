...
function canonicalDecisionText(value: string): string {
  return value.normalize("NFKC").toLocaleLowerCase()
    .replace(/[\s“”‘’'"`，。；：:、！？!?（）()【】\[\]{}]/gu, "")
}

function authoritativeDecisionText(sections: Record<string, string>, summary = ""): string {
...
function directlyRestatesOriginalRequest(line: string, request: string): boolean {
  ...
  if (lineText.length < 6 || requestText.length < 6) return false
  if (lineText === requestText) return true
  if (requestText.includes(lineText)) return true
  return lineText.includes(requestText) && lineText.length <= Math.ceil(requestText.length * 1.35)
}

function groupDecisionEntries(entries: Array<{ path: string; line: string }>): Map<string, Array<{ path: string; line: string }>> {
  const grouped = new Map<string, Array<{ path: string; line: string }>>()
  for (const entry of entries) grouped.set(entry.path, [...(grouped.get(entry.path) ?? []), entry])
  return grouped
}
...
function normalizedDecisionItems(raw: unknown, stageId: string): DecisionItem[] | undefined {
  ...
  return raw.map((item: any) => ({
    ...
    blocks: (Array.isArray(item?.blocks) ? item.blocks : []).map((block: any) => {
...
export function renderDecisionReviewSection(items: DecisionItem[]): string {
  ...
  if (deferred.length) lines.push("", "### 已明确延期或排除", ...deferred.map((item) =>
    `- ${item.id}：${item.question}（${item.status}${item.deferredToStage ? ` → ${item.deferredToStage}` : ""}）`))
  return lines.join("\n")
}
...
export function validateHumanDecisionContract(
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    if (item.status === "open") {
      ...
      for (let optionIndex = 0; optionIndex < item.options.length; optionIndex += 1) {
        ...
        if (OPTION_DEFERRAL_LANGUAGE.test(optionText) && !option.resultStatus) findings.push({
        ...
        if (option.resultStatus === "deferred" && !option.deferredToStage) findings.push({
    ...
    if (item.status === "deferred" && !item.deferredToStage) findings.push({
    ...
    for (let blockIndex = 0; blockIndex < item.blocks.length; blockIndex += 1) {
      ...
      if (blockIds.has(block.id)) findings.push({
      ...
      if (canonicalStatement.length >= 4 && authoritativeText.includes(canonicalStatement)) findings.push({
  ...
  const unboundEntries = decisionProseEntries.filter(({ line }) => items.some((item) =>
    line.includes(item.id) && !item.blocks.some((block) => line.includes(block.id))))
  for (const [entryPath, entries] of groupDecisionEntries(unboundEntries)) findings.push({
  ...
  if (stage.scopeContract?.id === "system-discovery") {
    const openOptionAssertions = decisionProseEntries.filter(({ line }) =>
      TARGET_BUSINESS_RULE_LANGUAGE.test(line)
    ...
    for (const [entryPath, entries] of groupDecisionEntries(openOptionAssertions)) findings.push({
    ...
    const untrackedTargetRules = decisionProseEntries.filter(({ line }) => {
      ...
      if (items.some((item) => line.includes(item.id)
        && item.blocks.some((block) => line.includes(block.id)))) return false
      if (/\bDEC-[A-Za-z0-9._-]+\b/u.test(line)) return false
      return !directlyRestatesOriginalRequest(line, state.originalRequest ?? "")
    })
    for (const [entryPath, entries] of groupDecisionEntries(untrackedTargetRules)) findings.push({
...
export async function prepare(input: PrepareInput): Promise<Transition & { stageCard: any }> {
  ...
  const stageCard = {
    ...
    ...(stage.decisionGate ? {
      ...
      deferredDecisions: (state.decisionLedger ?? []).filter((item) =>
        item.status === "deferred" && item.deferredToStage === stage.id),
    } : {}),
...
function repairWriteSetFindings(draft: StageDraft | undefined, input: SubmitInput): ValidationFinding[] {
  if (!draft?.validation?.signature) return []
  ...
  const touched = (entry: { code: string; path: string }): boolean => {
    if (entry.path === "summary") return Object.hasOwn(input, "summary") && Boolean(input.summary?.trim())
    if (entry.path === "sections") return Boolean(input.sections && Object.keys(input.sections).length)
    ...
    if (entry.path === "claims" || entry.path.startsWith("claims[")) return input.claims !== undefined
    if (entry.path === "decisionItems" || entry.path.startsWith("decisionItems[")) return input.decisionItems !== undefined
    return false
  }
...
export function validateStageSemantics(state: WorkflowState, stage: any, input: SubmitInput): ValidationFinding[] {
  ...
  if (stage.scopeContract?.id === "system-discovery") {
    ...
    if (state.workflowType === "add-feature" && stage.id === "02-big-picture-event-storm") {
      const misplacedCurrentAssertions: string[] = []
      const untracedCurrentCapabilities: string[] = []
      const baselineIds = (state.baselineClaims ?? []).map((claim) => claim.id).filter(Boolean)
      for (const [heading, text] of entries) {
        if (["输入场景与现状事实", "证据与追踪"].includes(heading)) continue
        let subsection = ""
        for (const rawLine of text.split(/\r?\n/u)) {
          const line = rawLine.trim()
          const subheading = line.match(/^#{3,6}\s+(.+)$/u)
          if (subheading) { subsection = subheading[1].trim(); continue }
          if (!/现状已存在/u.test(line)) continue
          if (!/能力状态分类/u.test(subsection)) misplacedCurrentAssertions.push(line)
          else if (!baselineIds.some((id) => line.includes(id))) untracedCurrentCapabilities.push(line)
        }
      }
      if (misplacedCurrentAssertions.length) addFinding(
        "STRATEGIC_TARGET_RULE_MASQUERADES_AS_CURRENT", "战略事件风暴",
        misplacedCurrentAssertions.slice(0, 4),
        "新增功能的大图阶段只能在能力状态分类或证据区声明现状能力；目标事件流、规则和异常不得借用相邻现状行为作为已批准目标语义",
      )
      if (untracedCurrentCapabilities.length) addFinding(
        "STRATEGIC_CURRENT_CAPABILITY_UNTRACED", "能力状态分类",
        untracedCurrentCapabilities.slice(0, 4),
        "能力状态分类中的现状能力必须引用 baselineClaims 的稳定 claim id，不能只凭代码印象或相邻接口推断",
      )
    }
    ...
    if (finalizedBoundaries.length) addFinding(
      "STRATEGIC_DISCOVERY_BOUNDARY_FINALIZED", "热点与边界线索", finalizedBoundaries.slice(0, 4),
      "Big Picture EventStorming 只能形成边界候选和验证线索；子域、限界上下文及职责边界必须留给战略设计批准",
    )
  }
  ...
  if (stage.scopeContract?.id === "context-tactical-design" && stage.id === "06-tactical-design") {
    ...
    if (aggregateOrmMerge.length) addFinding("TACTICAL_AGGREGATE_INFRASTRUCTURE_MERGE", "领域模型设计", aggregateOrmMerge,
      "请复核 ORM 映射是否侵入领域职责；是否使用独立映射模型由已批准的架构策略决定，不强制双模型")
    const original = state.originalRequest ?? ""
    ...
    if (moduleText && !contextFirstLayers.every((pattern) => pattern.test(moduleText))) {
      addFinding("TACTICAL_BOUNDED_CONTEXT_MODULE_INCOMPLETE", "模块与分层设计", ["domain/application/infrastructure/interfaces"],
        "请说明领域职责、用例编排和适配职责如何隔离；目录名称与物理分层服从项目架构决策，不强制四个目录")
    }
  }
