...
function mergeClaims(current: unknown, increment: unknown): unknown {
...
export function validateStageSemantics(state: WorkflowState, stage: any, input: SubmitInput): ValidationFinding[] {
  ...
  if (stage.scopeContract?.id === "system-discovery") {
    ...
    if (queryPseudoEvents.length) addFinding(
      "STRATEGIC_EVENT_NOT_STATE_CHANGE", "战略事件风暴", queryPseudoEvents,
    ...
    if (requiresScenarioClarification(state.originalRequest ?? "")) {
      ...
      if (resolution?.status !== "unresolved" || ids.size < 2 || !labelsComplete || affected.length === 0) {
        addFinding("AMBIGUOUS_SCENARIO_PREMATURE_COMMITMENT", "战略事件风暴", ["status=unresolved", "至少两项有稳定 id/label 的候选", "affectedDecisions"],
          "原始请求只有能力名称，尚不足以唯一确定触发、结果与规则；请用 stageCard.ambiguityContract.submitField 结构化声明候选与受影响决策，再把候选事件流并列交给人选择")
      }
    }
  }
