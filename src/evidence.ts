...
async function walkSources(
  ...
  catch { scan.unreadableDirectories += 1; return }
  for (const entry of entries) {
    if (ignored.has(entry.name) || ignoredFiles.has(entry.name)) continue
    if (entry.isSymbolicLink()) { scan.skippedSymlinks += 1; continue }
...
async function scanProjectSources(root: string, limit: number): Promise<SourceScan> {
...
function openSpecIndexStatement(currentSpecs: string[], priorChanges: string[]): string {
...
async function projectConventionEvidence(root: string): Promise<Array<{ file: string; excerpts: Array<{ ref: string; text: string; mandatory: boolean }> }>> {
  ...
  for (const candidate of conventionFiles) {
    ...
    if (!metadata || metadata.size > 160_000) continue
...
export async function evidenceBundle(
  ...
  if (terms.length < 2) throw new Error("evidence-bundle 需要 2-6 个稳定业务/代码关键词。")
  ...
  for (const file of files) {
    ...
    catch { scan.unreadableFiles += 1; continue }
    scannedTextFiles += 1
    if (!content) continue
