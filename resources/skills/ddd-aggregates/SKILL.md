---
name: ddd-aggregates
description: "在一个已选微服务及其限界上下文内，基于服务用例包和 Design-Level EventStorming 从不变量出发设计聚合根、实体、值对象、事务边界与跨聚合一致性。用于战术设计，禁止直接从表结构或 Big Picture EventStorming 推导聚合。"
---

# DDD Aggregates

本 Skill 只属于 `context-tactical-design`：消费已批准的 `III-tactical-eventstorm.md` 候选，在 `IV-tactical-design.md` 最终确定聚合与不变量。不得回头重划子域、限界上下文或部署边界，不得直接编辑代码。
...
- **必需**：已批准的微服务用例包（来自 `ddd-context-use-cases`）、Design-Level 事件流与命令/不变量候选（来自 `ddd-discover design-level`）、当前服务承载的上下文职责与词汇表（来自 `ddd-contexts`）。
