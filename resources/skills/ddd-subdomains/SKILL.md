---
name: ddd-subdomains
description: "从战略设计前已批准的 Big Picture EventStorming 识别业务能力并分类子域（Core/Supporting/Generic），产出核心域声明、业务投入与所有权建议。用于系统战略设计或现有系统的战略影响/目标边界分析，不消费 Design-Level 对象模型作为子域划分依据。"
---

# DDD Subdomains

本 Skill 只属于 `system-strategy`：消费 `I-strategic-eventstorm.md` 的业务能力与职责线索，在 `II-strategic-design.md` 分类核心/支撑/通用子域。不得设计聚合、服务、仓储、表或代码。
...
- **必需**：系统级用户场景，以及 Big Picture 的事件流、核心流程、能力与边界线索（来自 `ddd-discover big-picture`）。
