...
description: "在一个已选微服务及其限界上下文内，基于服务用例包、Design-Level EventStorming 和聚合设计领域事件、领域服务、仓储接口、工厂与跨聚合协作，并区分上下文内部领域事件和战略层集成事件。"
---

# DDD Domain Interactions

本 Skill 只属于 `context-tactical-design`：消费已批准的战术事件风暴与聚合设计，在 `IV-tactical-design.md` 确定上下文内部领域服务、领域事件、仓储接口、工厂和一致性协作。不得重做战略边界，也不得直接编码。
...
- **必需**：已批准的微服务用例包（来自 `ddd-context-use-cases`）、聚合目录与不变量表（来自 `ddd-aggregates`）、Design-Level 事件候选（来自 `ddd-discover design-level`）。
