...
description: "映射已批准限界上下文间的战略关系与集成策略：上下游方向、跨上下文事件、翻译模式、契约所有权、失败模式与版本策略，并将模块/微服务部署映射作为独立架构决策。"
---

# DDD Context Map

本 Skill 只属于 `system-strategy`：在 `II-strategic-design.md` 确定上下游、集成语义、契约所有权、失败与版本策略，并把部署映射作为独立决定。不得设计上下文内部聚合、服务、仓储、表或代码。
...
- **必需**：上下文目录（来自 `ddd-contexts`）、系统级用户场景，以及 Big Picture 事件流热点中的跨边界点（来自 `ddd-discover big-picture`）。
...
7. **子系统与部署映射决策**：划分子系统，明确核心流程和职责边界；分别评估每个上下文由哪个模块/微服务承载，并记录团队所有权、独立发布、扩缩容、故障隔离、数据自治、运维成本与 ADR。禁止默认一上下文一微服务。
...
| 部署映射 ADR    | 表格：上下文、模块/微服务选择、证据、收益、成本、触发拆分条件、回退方案 |
| 服务间事件流    | 表格：事件、发布服务/上下文、消费服务/上下文、核心流程、职责、契约 Owner |
