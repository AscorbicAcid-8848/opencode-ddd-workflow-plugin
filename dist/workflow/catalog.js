...
export async function intrinsics() {
...
export async function profileFor(type) {
    ...
    if (!source)
        throw new WorkflowError(`Unknown workflow type: ${type}`);
    ...
    for (const stage of profile.stages) {
        ...
        if (!contractId || !contract)
            throw new WorkflowError(`阶段缺少内禀属性契约：${type}/${stage.id}`);
...
export async function intrinsicFor(type, stageId) {
    const catalog = await intrinsics();
    const id = catalog.stageBindings?.[type]?.[stageId];
    const contract = catalog.contracts?.[id];
    if (!id || !contract)
        throw new WorkflowError(`阶段缺少内禀属性契约：${type}/${stageId}`);
    return [id, contract];
}
export function stageContract(profile, stageId) {
    const stage = profile.stages.find((item) => item.id === stageId);
    if (!stage)
        throw new WorkflowError(`Unknown stage: ${stageId}`);
    return stage;
}
export function stageIndex(profile, stageId) {
    const index = profile.stages.findIndex((item) => item.id === stageId);
    if (index < 0)
        throw new WorkflowError(`Unknown stage: ${stageId}`);
    return index;
}
export function validateTopology(profile) {
