...
async function walk(root, relative, output, limit) {
    ...
    for (const entry of await readdir(absolute, { withFileTypes: true })) {
        if (output.length >= limit || ignored.has(entry.name))
            continue;
        const child = path.join(relative, entry.name);
        if (entry.isDirectory())
            await walk(root, child, output, limit);
...
export async function evidenceBundle(projectRoot, workflowId, rawTerms) {
    ...
    if (terms.length < 2)
        throw new Error("evidence-bundle 需要 2-6 个稳定业务/代码关键词，例如 Shop、UserHolder、view、trail。");
    ...
    for (const root of sourceRoots)
        await walk(projectRoot, root, files, 500);
    const lowered = searchTerms;
    const seenTerms = new Set();
    const matches = [];
    for (const file of files) {
        const absolute = path.join(projectRoot, file);
        if ((await stat(absolute)).size > 160_000)
            continue;
