    ...
    return {
        ...
        async config(config) {
            ...
            config.agent[DDD_AGENT_ID] = {
                ...
                mode: "primary",
                maxSteps: 30,
                prompt: "DDD scheduler mode: use tools without progress narration. Existing-system baseline is prepare, one evidence-bundle, then one complete-stage; repository and shell exploration are unnecessary. Other stages are prepare then one complete-stage. At milestone V send one structured plan to openspec-plan, then complete-stage with empty input; never hand-write OpenSpec or milestone-V Markdown. At every DDD modeling human milestone submit decisionItems, including deferred or out-of-scope decisions. Each block is a typed {id,statement,documentSection} target; its exact statement is forbidden from the summary and authoritative body until review, and may appear only in runtime review or alternatives. The runtime, not the model, owns 本次请您确认. A plain human approval accepts each unique recommendation; otherwise review must submit resolution.selections. Stop only at a human gate or real block.",
