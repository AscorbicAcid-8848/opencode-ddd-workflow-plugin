...
async function resolveActiveIdentity(ctx, workflowType, workflowId) {
    ...
    if (workflowType && workflowId) {
        ...
        if (ctx.sessionID)
            sessionIdentities.set(ctx.sessionID, resolved);
        return resolved;
    }
    ...
    if (candidates.length === 0)
        throw new Error("当前项目没有活动的 DDD change；请先用 action=init 创建。");
...
const submitSectionTool = tool({
    ...
    async execute(args, context) {
        try {
            ...
            return out({ error: error.message, errorType: error.name });
        }
    },
});
...
export const DddWorkflowPlugin = async (pluginInput, pluginOptions) => {
    ...
    return {
        async "chat.message"(input, output) {
            ...
            if (!originalRequest && !persistedStage && !modelingAgent && !codingAgent)
                return;
        ...
        async config(config) {
            ...
            config.agent[DDD_AGENT_ID] = {
                ...
                mode: "primary",
                maxSteps: 30,
                prompt: "DDD scheduler mode: use tools without progress narration. Existing-system baseline is prepare, one evidence-bundle, then one complete-stage; repository and shell exploration are unnecessary. Other stages are prepare then one complete-stage. At milestone V send one structured plan to openspec-plan, then complete-stage with empty input; never hand-write OpenSpec or milestone-V Markdown. At every DDD modeling human milestone submit decisionItems, including deferred or out-of-scope decisions. Each block is a typed {id,statement,documentSection} target; its exact statement is forbidden from the summary and authoritative body until review. Any authoritative mention of an open/deferred issue cites both DEC-ID/BLOCK-ID on the same line. The runtime, not the model, owns 本次请您确认. A plain human approval accepts each unique recommendation; otherwise review must submit resolution.selections. Stop only at a human gate or real block.",
        ...
        async "tool.execute.before"(input, hookOutput) {
            ...
            if (activeStage && implementationStages.has(activeStage)) {
                ...
                if (["bash", "shell"].includes(toolName)) {
                    ...
                    if (used >= 10)
                        throw new Error("DDD_IMPLEMENTATION_COMMAND_BUDGET_EXHAUSTED: 当前切片 10 次命令预算已用完。禁止通过命令变体反复重试；请提交已有真实证据或调用 action=block。");
        ...
        async "tool.execute.after"(input, hookOutput) {
            ...
            if (!dddSessions.has(input.sessionID) || request.action !== "prepare")
                return;
            try {
