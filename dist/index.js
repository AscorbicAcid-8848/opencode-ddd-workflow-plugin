    ...
    async execute(args, context) {
        try {
            ...
            if (args.action === "status") {
                ...
                if (turnIntents.get(context.sessionID) === "read-only")
                    return out({ ...result,
...
export const DddWorkflowPlugin = async (pluginInput, pluginOptions) => {
    ...
    return {
        async "chat.message"(input, output) {
            ...
            if (!panel && !originalRequest && !persistedStage && !modelingAgent && !codingAgent)
                return;
        ...
        async "command.execute.before"(input, output) {
            ...
            if (input.command === "ddd" || input.command === "ddd-code")
                turnIntents.set(input.sessionID, classifyTurn(`/ddd ${input.arguments ?? ""}`));
            if (input.command === "ddd" || input.command === "ddd-code")
                dddSessions.add(input.sessionID);
        ...
        async "tool.execute.before"(input, hookOutput) {
            ...
            if (["edit", "write", "apply_patch", "patch", "multiedit", "multi_edit"].includes(toolName)) {
                ...
                if (sessionIsDdd && target && formalArtifact.test(target)) {
                    ...
                    if (args) {
                        if ("filePath" in args)
                            args.filePath = path.resolve(pluginInput.directory);
                        if ("path" in args)
                            args.path = path.resolve(pluginInput.directory);
