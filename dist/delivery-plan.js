...
const DELTAS = new Set(["ADDED", "MODIFIED"]);
...
const meaningful = (value) => Boolean(value.trim()) && !PLACEHOLDER.test(value.trim());
...
function normalizeDelta(value, prior) {
    if (value === undefined)
        return prior;
    const normalized = text(value).toUpperCase();
    return (normalized || undefined);
}
function normalizeBehaviorProtection(value, prior) {
    if (value === undefined && !prior)
        return undefined;
    const raw = record(value) ?? {};
    const array = (field) => raw[field] === undefined
        ? [...(prior?.[field] ?? [])]
        : list(raw[field]);
    return {
...
function normalizeRollback(value, prior) {
    if (value === undefined && prior)
        return {
...
function defaultDelta(context) {
    return context.workflowType === "refactor-system" ? "MODIFIED" : "ADDED";
}
function effectiveDelta(capability, context) {
    return capability.delta ?? defaultDelta(context);
}
function rollbackComplete(rollback) {
...
function behaviorProtectionComplete(protection) {
...
export function validateStructuredPlan(plan, context = {}) {
    const findings = [];
    const required = (value, path, label) => {
        if (!meaningful(value))
            findings.push({ code: "PLAN_FIELD_REQUIRED", path, message: `${label}不能为空或使用占位词。` });
    };
    required(plan.title, "plan.title", "计划标题");
    required(plan.objective, "plan.objective", "业务目标");
    const isRefactor = context.workflowType === "refactor-system";
    if (context.skipSpecs && !isRefactor)
        findings.push({
            code: "PLAN_SKIP_SPECS_WORKFLOW_INVALID", path: "skipSpecs",
            message: "skipSpecs 只允许行为保持型 refactor-system。",
        });
    if (isRefactor && context.skipSpecs && plan.capabilities.length > 0)
        findings.push({
    ...
    if (!plan.capabilities.length && !(isRefactor && context.skipSpecs))
        findings.push({
    ...
    plan.slices.forEach((slice, index) => {
        ...
        for (const [field, label] of [["title", "标题"], ["outcome", "可观察结果"], ["consumer", "真实消费者"], ["compatibility", "兼容策略"]]) {
        ...
        for (const [field, label] of [["acceptanceCriteria", "验收标准"], ["modelElementIds", "ME 模型元素"], ["invariantIds", "INV 不变量"], ["productionPaths", "生产路径"], ["testPaths", "测试路径"], ["verification", "验证命令"]]) {
            if (!slice[field].length || slice[field].some((item) => !meaningful(item)))
                findings.push({
                    code: "PLAN_SLICE_LIST_REQUIRED", path: `${base}.${field}`, message: `切片必须声明真实${label}，不能使用占位词。`,
                });
        }
        if (!meaningful(slice.rollback.trigger))
            findings.push({
                code: "PLAN_ROLLBACK_TRIGGER_REQUIRED", path: `${base}.rollback.trigger`,
                message: "回滚合同必须声明可观察触发条件。",
            });
        if (!slice.rollback.steps.length || slice.rollback.steps.some((item) => !meaningful(item)))
            findings.push({
                code: "PLAN_ROLLBACK_STEPS_REQUIRED", path: `${base}.rollback.steps`,
                message: "回滚合同必须声明有序且可执行的步骤。",
            });
        if (!slice.rollback.verification.length || slice.rollback.verification.some((item) => !meaningful(item)))
            findings.push({
        ...
        if (isRefactor) {
            ...
            if (!protection)
                ...
                for (const [field, label] of [["baselineScenarioRefs", "基线场景引用"], ["characterizationTests", "特征测试"], ["preservedSemantics", "保留语义"]]) {
                    if (!protection[field].length || protection[field].some((item) => !meaningful(item)))
                        findings.push({
                            code: "PLAN_BEHAVIOR_PROTECTION_FIELD_REQUIRED", path: `${base}.behaviorProtection.${field}`,
                            message: `重构切片必须声明真实${label}。`,
                        });
                }
                if (!meaningful(protection.coexistenceStrategy))
                    findings.push({
...
function deltaLabel(delta) {
    return { ADDED: "新增", MODIFIED: "调整" }[delta];
}
function formatRollbackInline(rollback) {
    return `触发：${rollback.trigger}；步骤：${rollback.steps.join("；")}；恢复验证：${rollback.verification.join("；")}`;
}
function formatBehaviorProtectionInline(protection) {
    if (!protection)
        return "未提供结构化行为保护合同";
    return [
...
export function compileStructuredPlan(plan, workflowId, context = {}) {
...
export function compileDeliveryMilestoneSections(plan, workflowId, contract = {}, context = {}) {
    ...
    const requirementRows = plan.capabilities.flatMap((capability) => capability.requirements.flatMap((requirement) => requirement.scenarios.map((scenario) => `- ${capability.id} [${effectiveDelta(capability, context)}] / ${requirement.name} / ${scenario.name}：WHEN ${scenario.when}；THEN ${scenario.then}`)));
    ...
    const sections = {
        ...
        "Git 交付计划": [
            ...
            ...plan.slices.map((slice) => `- ${slice.id}：验证通过后独立提交；触发“${slice.rollback.trigger}”时执行：${slice.rollback.steps.join("；")}；随后验证：${slice.rollback.verification.join("；")}。`),
