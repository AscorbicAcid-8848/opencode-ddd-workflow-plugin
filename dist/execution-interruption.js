...
export async function readExecutionInterruption(root, state) {
    try {
...
export function isInterruptedReply(messages) {
    ...
    if (last.info.error?.name === 'MessageAbortedError')
        return undefined;
    if (last.parts.some((p) => p.type === 'tool' || p.type === 'text' && p.text?.trim()))
        return undefined;
    if (!['unknown', 'length', 'stop'].includes(last.info.finish) && !last.info.error)
        return undefined;
    return last.info;
}
/** No model calls or business state mutations. Idle is not workflow completion. */
export function createInterruptionMonitor(project, client) {
    ...
    return async (sessionID) => {
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (transition.stopAllowed !== false || Date.parse(state.updatedAt) > info.time.completed)
                    continue;
                const old = await readExecutionInterruption(item.root, state);
                if (old?.messageID === info.id)
                    continue;
...
//# sourceMappingURL=execution-interruption.js.map