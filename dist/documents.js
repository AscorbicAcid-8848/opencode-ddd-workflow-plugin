...
function readerSections(body) {
    ...
    for (const line of body.split(/(?<=\n)/u)) {
        ...
        if (marker) {
            ...
            if (heading)
                headings.push({ level: heading[1].length, label: heading[2], start: offset, content: offset + line.length });
        }
        offset += line.length;
    }
...
export function renderReaderDocument(title, milestone, sections) {
    const buckets = READER_PARTS.map(() => []);
...
export function writableHeadingsForStage(stage) {
    ...
    if (explicit)
        return [...new Set(explicit)].filter((heading) => !OVERVIEW_HEADINGS.includes(heading) && !REVIEW_HEADINGS.includes(heading));
    // Unknown stages fail closed instead of silently gaining write access to a
    // complete milestone document.
    return [];
}
export function stageArtifactPath(root, stage) {
    return path.join(root, ".ddd", "stages", `${stage.id}.md`);
}
async function generateStageSkeleton(profile, stage) {
    ...
    const lines = [`# 阶段 ${stage.id}：${String(stage.artifactTitle ?? stage.id)}`, ""];
    for (const heading of allowed) {
        lines.push(`## ${heading}`, "");
        for (const subsection of contracts.get(heading) ?? [])
            lines.push(`### ${subsection}`, "", "> _待填写_", "");
    }
    return lines.join("\n").replace(/\n{3,}/gu, "\n\n").trimEnd() + "\n";
}
export async function candidateStageDocument(root, profile, stage, sections) {
...
export async function publishStageSections(root, profile, stage, sections) {
    const file = stageArtifactPath(root, stage);
    await atomicText(file, await candidateStageDocument(root, profile, stage, sections));
    return file;
}
export async function compileMilestoneDocument(root, profile, summaryStage, summaries, decisionReview) {
    ...
    for (const item of summaries) {
        for (const [heading, content] of Object.entries(item.sections)) {
            if (!milestoneHeadings.has(heading) || !content.trim())
                continue;
