...
export async function writeLink(root, state, status, changeId, archiveTarget) {
    const link = {
...
export async function verifyArchive(projectRoot, id) {
    try {
        ...
        if (!await exists(change))
            return { archived: false, error: `OpenSpec active change not found: ${id}` };
...
async function ensureChangeMetadata(projectRoot, id, skipSpecs) {
...
async function specFiles(change) {
...
export async function planningArtifacts(projectRoot, id) {
    const change = activeChange(projectRoot, id);
    const files = ["proposal.md", "design.md", "tasks.md"];
    const missing = [];
    for (const file of files)
        if (!await exists(path.join(change, file)))
            missing.push(file);
...
export async function openSpecAction(input) {
    ...
    if (content !== undefined || skipSpecs !== undefined) {
        if (artifact === "apply")
            throw new WorkflowError("apply 只用于严格校验，不能写入内容。");
        if (skipSpecs) {
        ...
        if (!content?.trim())
            throw new WorkflowError(`${artifact} 工件内容不能为空。`);
        let file;
        if (artifact === "specs") {
