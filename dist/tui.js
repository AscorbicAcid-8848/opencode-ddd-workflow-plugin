...
const defaults = { key: "", milestone: 0, tab: "可视化", query: "", status: "全部", type: "全部" };
/** The native route is independently exportable for real OpenTUI render tests. */
export function createDashboard(api, project, leave) {
    ...
    const notify = (message, variant = "info") => api.ui.toast({ message, variant });
    ...
    async function select(item) {
        ...
        clear(controls);
        if (!item) {
            card("没有匹配的 Workflow", "调整筛选；项目尚无流程时，可返回会话使用 /ddd <需求> 创建。");
            return;
        }
        pref.key = item.key;
        save();
    ...
    function renderDetail() {
        ...
        clear(body);
        clear(controls);
        const shortStatus = (value) => ({ "已批准": "已批", "待审核": "待审", "要求修改": "待改", "未开始": "未始", "已拒绝": "拒绝", "形成中": "生成" })[value] ?? value;
        for (let i = 0; i < 6; i++)
        ...
        for (const name of ["可视化", "结论", "决策", "正文", "历史"])
            button(tabs, name, () => { pref.tab = name; save(); renderDetail(); }, name === pref.tab ? theme.primary : theme.textMuted);
        ...
        if (selected.legacy)
            card("旧版记录 · 只读", "已兼容显示旧版文档与人工审核证据；不会通过面板迁移或修改旧版工作流。", theme.textMuted);
        if (pref.tab === "可视化") {
            ...
            card("正式里程碑文档", `${view.file}\n\n${view.body || "本里程碑尚未形成正式文档。"}`);
        else if (pref.tab === "历史") {
            for (const ref of view.revisions ?? [])
                button(body, `查看投影版本 ${ref.revision} · ${ref.status}`, () => {
            ...
            if (view.tasks)
                card("OpenSpec Tasks", view.tasks);
        }
    ...
    function startReview(decision) {
        ...
        function chooseNext(i) {
            if (i < open.length) {
                ...
                api.ui.dialog.replace(() => createComponent(api.ui.DialogPrompt, { title: "请填写具体反馈", onConfirm: feedback => {
                        if (!feedback.trim()) {
    ...
    root.onKeyDown = event => {
        ...
        if (event.name === "escape")
            ...
            const names = ["可视化", "结论", "决策", "正文", "历史"];
            pref.tab = names[(names.indexOf(pref.tab) + 1) % names.length];
            save();
            renderDetail();
        }
...
export const tui = async (api) => {
    if (!api.route?.register || !api.keymap?.registerLayer || !api.ui?.dialog) {
        api.ui?.toast?.({ message: "当前宿主不支持 DDD TUI 控制面板", variant: "warning" });
        return;
    }
