...
import { discoverWorkflows } from "./model.js";
...
export async function projectSessions(project, client) {
    const result = await client.session.list({ directory: project });
    if (result.error || !result.data)
        throw new Error("无法读取项目会话");
    const sessions = [];
    for (const session of result.data) {
        if (session.time.archived || session.parentID)
            continue;
        try {
            if (await sameDirectory(session.directory, project))
...
export async function resumeInSession(item, target, client) {
    const key = `${item.project}:${item.key}`;
    if (inFlight.has(key))
        throw new Error("工作流正在提交续跑请求");
    inFlight.add(key);
    try {
        const fresh = (await discoverWorkflows(item.project)).find(w => w.key === item.key);
        if (!fresh?.state || fresh.archived || fresh.legacy || fresh.issues.length || ["complete", "rejected"].includes(fresh.state.status))
            throw new Error("该工作流不可续跑，请刷新查看");
        const statuses = await client.session.status({ directory: item.project });
        if (statuses.error || !statuses.data)
            throw new Error("无法确认会话是否空闲");
        const linked = new Set([fresh.state.runtimeSessionId, ...(fresh.state.runtimeSessionIds ?? [])]);
        for (const id of linked) {
            if (id && statuses.data[id] && statuses.data[id].type !== "idle")
                throw new Error("该工作流的执行会话正在运行，请等待完成");
        }
        let sessionID = target;
        if (sessionID) {
            const session = await client.session.get({ sessionID, directory: item.project });
            if (session.error || !session.data || session.data.time.archived || session.data.parentID || !await sameDirectory(session.data.directory, item.project))
                throw new Error("目标会话不可用或不属于当前目录");
            if (statuses.data[sessionID] && statuses.data[sessionID].type !== "idle")
                throw new Error("目标会话正在运行，请稍后再试");
        }
        else {
        ...
        if (sent.error)
            throw new Error(`续跑发送失败，请检查目标会话后再操作：${sessionID}`);
        return sessionID;
    }
    finally {
        inFlight.delete(key);
