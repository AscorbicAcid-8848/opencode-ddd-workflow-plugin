...
export function resolveLanguage(host, env = process.env, fallback) {
    ...
    for (const value of [config?.locale, config?.language, env.LC_ALL, env.LC_MESSAGES, env.LANGUAGE?.split(":")[0], env.LANG]) {
        if (typeof value !== "string" || !value || value === "auto" || /^(C|POSIX)(\.|$)/iu.test(value))
            continue;
...
export function translate(language, text) { return language === "en" ? en[text] ?? text : text; }
