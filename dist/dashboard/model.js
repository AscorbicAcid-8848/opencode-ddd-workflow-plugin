...
export const typeLabels = { "add-feature": "新增功能", "refactor-system": "系统重构", "create-system": "新建系统" };
...
export async function safeRead(root, relative) {
    ...
    if (lexical.startsWith("..") || path.isAbsolute(lexical))
        throw new Error("文档路径超出当前 change");
    const file = await realpath(requested);
    const rel = path.relative(base, file);
    if (rel.startsWith("..") || path.isAbsolute(rel))
        throw new Error("文档路径超出当前 change");
    if ((await stat(file)).size > 2 * 1024 * 1024)
        throw new Error("文档超过 2 MB，无法在面板中加载");
    return readFile(file, "utf8");
}
async function folders(root) {
    try {
        return (await readdir(root, { withFileTypes: true })).filter(d => d.isDirectory() && !d.isSymbolicLink()).map(d => d.name);
    }
    catch (error) {
...
export async function readWorkflow(project, root, archived) {
    const result = { key: path.relative(project, root), root, project, archived,
    ...
    try {
        ...
        if (!data || !Array.isArray(data.checkpoints) || typeof data.workflowId !== "string" || !typeLabels[data.workflowType])
            throw new Error("无法识别的 workflow state");
        const state = data;
        ...
        if (result.legacy)
            state.checkpoints = data.checkpoints.map((c) => {
        ...
        if (!archived && path.basename(path.dirname(root)) !== state.workflowId)
            result.issues.push("活动目录与 workflow ID 不一致");
        if (archived && state.status !== "complete")
            result.issues.push("目录已归档，但状态未完成");
        if (!archived && state.status === "complete")
            result.issues.push("状态已完成，但目录尚未归档");
        if (archived && result.approved !== 6)
            result.issues.push("归档记录缺少完整的六里程碑批准证据");
...
export async function discoverWorkflows(project) {
    ...
    try {
        ...
        if (rel.startsWith("..") || path.isAbsolute(rel))
            throw new Error("OpenSpec 目录不属于当前项目");
    }
    catch (error) {
        if (error.code === "ENOENT")
            return [];
...
export async function loadMilestone(item, index) {
    const roman = romans[index];
    if (!roman || !item.profile)
        throw new Error("无效的里程碑或工作流");
    const name = item.profile.documents[`milestone${roman}`];
    const view = { roman, title: titles[index], status: milestoneStatus(item, roman),
    ...
    if (view.checkpoint) {
        try {
            view.body = await safeRead(item.root, name);
    ...
    view.revisions = item.state.dashboard?.revisions.filter(r => r.roman === roman) ?? [];
    const latestProjection = view.revisions.at(-1);
    if (latestProjection) {
        try {
            const projection = await readProjection(item, latestProjection);
            if (projection.documentHash !== createHash("sha256").update(view.body).digest("hex"))
                throw new Error("正式文档与已发布投影不一致");
            if (latestProjection.checkpointId !== view.checkpoint?.checkpointId || projection.status !== view.checkpoint?.status)
                throw new Error("当前检查点与已发布投影不一致");
            view.projection = projection;
        }
        catch (error) {
            view.issues.push(`可视化投影：${String(error)}`);
        }
    }
    // Engineering artifacts are relevant only from their owning milestones onward.
    for (const [key, file, since] of [["model", "model-contract.json", 3], ["plan", ".ddd/delivery/plan.json", 4]]) {
        if (index < since)
            continue;
        try {
            view[key] = JSON.parse(await safeRead(item.root, file));
        }
        catch (error) {
            if (error.code !== "ENOENT")
                view.issues.push(`${file}：${String(error)}`);
        }
    }
    if (index >= 4) {
        try {
            view.tasks = await safeRead(path.dirname(item.root), "tasks.md");
        }
        catch (error) {
            if (error.code !== "ENOENT")
                view.issues.push(`tasks.md：${String(error)}`);
        }
    }
    return view;
}
export async function readProjection(item, ref) {
    const projection = JSON.parse(await safeRead(item.root, ref.file));
    if (projection.version !== 1 || projection.workflowId !== item.id || projection.milestoneRoman !== ref.roman
        || projection.revision !== ref.revision || !Array.isArray(projection.cards))
        throw new Error("投影身份或版本不匹配");
    return projection;
}
export async function historicalMilestone(item, current, ref) {
