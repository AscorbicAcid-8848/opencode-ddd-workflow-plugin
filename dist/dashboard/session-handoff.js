...
export function reviewMessageId(workflowId, checkpointId, record) {
...
export async function resolvePanelReview(project, sessionID, messageID) {
    if (!messageID || !/^msg_[a-f0-9]{32}$/u.test(messageID))
        return undefined;
    for (const item of await discoverWorkflows(project)) {
        const state = item.state;
        if (!state || ![state.runtimeSessionId, ...(state.runtimeSessionIds ?? [])].includes(sessionID))
            continue;
        for (const checkpoint of state.checkpoints) {
            const record = checkpoint.review;
            if (record?.reviewer !== "user:tui" || reviewMessageId(item.id, checkpoint.checkpointId, record) !== messageID)
                continue;
...
export async function sendPanelReview(item, previous, client) {
    const fresh = (await discoverWorkflows(item.project)).find(w => w.key === item.key);
    if (!fresh || fresh.archived)
        throw new Error("工作流已移动或归档，不能自动继续。");
    const view = await loadMilestone(fresh, romans.indexOf(previous.roman));
    ...
    try {
        ...
        if (existing.data)
            return { sessionID, messageID, alreadySent: true };
        if (existing.response?.status !== 404)
            throw new Error("无法确认审核消息是否已发送。请检查原会话后再重试。");
        if (fresh.state?.checkpoints.at(-1)?.checkpointId !== checkpoint.checkpointId)
            throw new Error("工作流已进入后续阶段，不再补发旧审核的继续指令。");
        const statuses = await client.session.status({ directory: item.project });
        if (statuses.error || !statuses.data)
            throw new Error("无法读取原会话状态，请稍后重试同步。");
        if (statuses.data[sessionID] && statuses.data[sessionID].type !== "idle")
            throw new Error("原会话正在运行。审核已保存，请在会话空闲后重试同步。");
        ...
        if (result.error)
            throw new Error("审核已保存，但消息发送失败。请重试同步，不要重复审核。");
