...
export async function validateStageClaims(state, scopeId, writableHeadings, sections, rawClaims) {
    ...
    const authorizedSearch = new Map((Array.isArray(snapshot.authorizedSearchEvidence)
        ? snapshot.authorizedSearchEvidence : []).map((item) => [String(item?.ref ?? ""),
        ...
        for (const reference of claim.evidenceRefs) {
            ...
            if (reference.startsWith("search:") && reference !== "search:openspec/specs-and-prior-changes") {
                ...
                if (!absentTerms) {
                    ...
                    findings.push(finding("SEARCH_EVIDENCE_SUBJECT_MISMATCH", `${base}.statement`, `该负向搜索只证明这些精确代码词未命中：${absentTerms.join("、")}；不能据此扩大为其他能力或实体不存在。`));
                }
            }
    ...
    for (const sentence of sentences(narrative)) {
        if (isUnprovenTargetBehavior(sentence, claims)) {
            findings.push(finding("EVIDENCE_STAGE_TARGET_BEHAVIOR_LEAK", "sections", `现状证据阶段把尚未批准的新功能行为写成了验收规则：${sentence.slice(0, 120)}`, "本阶段只记录 AS-IS 行为与兼容约束；新功能触发、结果、授权和异常语义请改为 open-question，交由战略事件风暴和人工里程碑决定。"));
        }
        if (hasUnnegatedDesignDecision(sentence) && !isProvenCurrentDecision(sentence, claims) && !isDeclaredUncertainty(sentence, claims)) {
            findings.push(finding("EVIDENCE_STAGE_TARGET_DESIGN_LEAK", "sections", `现状证据阶段包含未经当前事实证明的目标设计或交付决定：${sentence.slice(0, 120)}`, "删除该决定，或改为 evidence-gap/open-question；目标方案留给战略、战术或交付阶段。"));
        }
    }
    return findings;
}
