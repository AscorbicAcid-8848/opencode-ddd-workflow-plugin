...
function hasUnnegatedDesignDecision(text) {
    ...
    if (/^(?:原始)?(?:请求|需求)(?:要求|提及|是|为|包含|明确)/u.test(compact))
        return false;
    if (/(?:假设|待确认|未证实|尚未证实|证据缺口|evidence-gap|open-question)/iu.test(compact))
        return false;
    // A constraint on a hypothetical new artifact is still target design even
    // when phrased negatively ("the new table must not...").
    if (/(?:新增|新建)[^。；\n]{0,24}(?:表|接口|缓存|Redis|模块)[^。；\n]{0,36}(?:不得|不应|不能|避免)/iu.test(compact))
        return true;
    if (/(?:Redis|缓存|新增接口)[^。；\n]{0,30}(?:需|须|必须|应当|应该)[^。；\n]{0,20}(?:避免|采用|使用|遵循)/iu.test(compact))
        return true;
    if (/(?:新增[^。；\n]{0,16}|新功能[^。；\n]{0,16}|轨迹记录[^。；\n]{0,16})(?:应|需|须|必须)[^。；\n]{0,36}(?:异步|低延迟|Redis|缓存|接口路径|数据表|持久化)/iu.test(compact))
        return true;
    if (/(?:不|未|无需|禁止|不得|不应|不能)(?:引入|新增|新建|拆分|迁移|采用|使用|改变|修改)/u.test(compact))
        return false;
    const patterns = [
...
function isDeclaredUncertainty(sentence, claims) {
    ...
    if (disguisedDecision)
        return false;
    return claims.some((claim) => ["evidence-gap", "open-question"].includes(claim.kind)
        && (sentence.includes(claim.statement) || claim.statement.includes(sentence)));
}
function isUnprovenTargetBehavior(sentence, claims) {
    const compact = sentence.replace(/\s+/gu, "");
    ...
    if (!behaviorContract)
        return false;
    if (/(?:待确认|未明确|尚未决定|是否|候选|开放问题|evidence-gap|open-question)/iu.test(compact))
        return false;
...
export async function validateStageClaims(state, scopeId, writableHeadings, sections, rawClaims, summary = "") {
    ...
    if (!Array.isArray(rawClaims) || rawClaims.length === 0) {
        return [finding("STAGE_CLAIMS_REQUIRED", "claims", "现状证据阶段必须提交类型化 claims，不能只提交自由 Markdown。", "按 stageCard.claimContract 提交事实、兼容约束、证据缺口或未决问题。")];
    }
    const claims = rawClaims;
    const snapshotFile = path.join(state.artifactRoot, ".ddd", "workbench", "evidence-snapshot.json");
    const snapshot = await exists(snapshotFile) ? await readJson(snapshotFile) : {};
    const authorizedSearch = new Map((Array.isArray(snapshot.authorizedSearchEvidence)
        ? snapshot.authorizedSearchEvidence : []).map((item) => {
        const absentTerms = Array.isArray(item?.absentTerms) ? item.absentTerms.map(String) : [];
        const fallback = `本次完整源码文件扫描未命中这些精确代码词：${absentTerms.map((term) => `\`${term}\``).join("、")}。`;
    ...
    for (let index = 0; index < claims.length; index += 1) {
        ...
        if (!claim || typeof claim !== "object" || Array.isArray(claim)) {
            findings.push(finding("CLAIM_NOT_OBJECT", base, "claim 必须是对象。"));
            continue;
        }
        for (const key of ["id", "kind", "statement", "maturity", "documentSection"]) {
            if (!nonEmpty(claim[key]))
                findings.push(finding("CLAIM_FIELD_REQUIRED", `${base}.${key}`, `${key} 不能为空。`));
        }
        if (nonEmpty(claim.id)) {
            if (ids.has(claim.id))
                findings.push(finding("CLAIM_ID_DUPLICATED", `${base}.id`, `claim id 重复：${claim.id}。`));
            ids.add(claim.id);
        }
        ...
        if (!Array.isArray(claim.authorityRefs) || claim.authorityRefs.length === 0) {
            ...
            for (const reference of claim.authorityRefs) {
        ...
        for (const reference of claim.evidenceRefs) {
            ...
            if (reference.startsWith("search:") && reference !== "search:openspec/specs-and-prior-changes") {
                ...
                if (!authorization) {
                    findings.push(finding("SEARCH_EVIDENCE_NOT_ISSUED", `${base}.evidenceRefs`, `负向搜索引用不是 evidence-bundle 签发的证据：${reference}。`));
                }
        ...
        if (isDomainAbsenceAssertion(absenceCandidate)) {
            if (attributes.availability !== "absent" || !claim.evidenceRefs.some((reference) => reference.startsWith("search:"))) {
                findings.push(finding("ABSENCE_CLAIM_NOT_PROVEN", base, "“不存在、只有或仅有”属于负向事实，必须标记 availability=absent 并提供 search:范围 证据。"));
            }
        }
    ...
    for (const part of narrativeParts)
        for (const sentence of sentences(part.text)) {
            ...
            const mappedAbsence = claims.some((claim) => sentence.includes(claim.statement) || claim.statement.includes(sentence));
            if (isDomainAbsenceAssertion(absenceCandidate) && !mappedAbsence)
                findings.push(finding("UNMAPPED_ABSENCE_ASSERTION", part.path, `正文或摘要包含未登记为 claim 的不存在性结论：${sentence.slice(0, 120)}`, "不存在性结论必须由 evidence-bundle 签发的精确负向搜索 statement 支持；否则改写为证据范围或开放问题。"));
            if (isUnprovenTargetBehavior(sentence, claims)) {
                findings.push(finding("EVIDENCE_STAGE_TARGET_BEHAVIOR_LEAK", part.path, `现状证据阶段把尚未批准的新功能行为写成了验收规则：${sentence.slice(0, 120)}`, "本阶段只记录 AS-IS 行为与兼容约束；新功能触发、结果、授权和异常语义请改为 open-question，交由战略事件风暴和人工里程碑决定。"));
            }
            if (hasUnnegatedDesignDecision(sentence) && !isProvenCurrentDecision(sentence, claims) && !isDeclaredUncertainty(sentence, claims)) {
                findings.push(finding("EVIDENCE_STAGE_TARGET_DESIGN_LEAK", part.path, `现状证据阶段包含未经当前事实证明的目标设计或交付决定：${sentence.slice(0, 120)}`, "删除该决定，或改为 evidence-gap/open-question；目标方案留给战略、战术或交付阶段。"));
            }
        }
    return findings;
}
